...
@
\caption{\label{fig:m1Profs}MCAP confidence intervals for Model 1 parameters.}
\end{figure}
...
\hline
  ...
  Process noise & $\sigmaProc: \mathrm{Epi}$ & $\Sexpr{myround(h1_mles %>% filter(prof_var == 'sig_sq_epi') %>% pull(value), 3)}$
   &
...
\hline
  ...
  Process noise & $\sigmaProc: \mathrm{End}$ & $\Sexpr{myround(h1_mles %>% filter(prof_var == 'sig_sq_end') %>% pull(value), 3)}$
   &
...
@
\caption{\label{fig:m2Profs}MCAP confidence intervals for Model 2 parameters.}
\end{figure}
...
\hline
 ...
 Human to water shedding & $\Wshed$ & $\Sexpr{myround(h2_mles['Mu'], 1)}$
   &
  $(\Sexpr{myround(h2_plots_lb['Mu'], 1)}, \Sexpr{myround(h2_plots_ub['Mu'], 1)})$ 
\\
\hline

 Water to Human Infection & $\beta_W$ & $\Sexpr{myround(h2_mles['BetaW'], 3)}$
   &
...
Human to Human Infection & $\transmission$ & $\Sexpr{signif(h2_mles['Beta'], 3)}$\textsuperscript{*}
   &
  $[0, \Sexpr{signif(h2_plots_ub['Beta'], 3)})$ 
\\
\hline

Immunity (yr) & $\muRS^{-1}$ & $\Sexpr{signif(1/h2_mles['sigma'], 3)}$\textsuperscript{*}
   &
...
@
\caption{\label{fig:m3Profs}MCAP confidence intervals for Model 3 parameters.}
\end{figure}
...
\hline
 ...
 Process Noise & $\sigmaProc$ & $\Sexpr{myround(h3_mles %>% filter(prof_var == 'std_W') %>% pull(value), 3)}$
   &
  $(\Sexpr{myround(all_ci %>% filter(prof_var == 'std_W') %>% pull(lower), 3)}, \Sexpr{myround(all_ci %>% filter(prof_var == 'std_W') %>% pull(upper), 3)})$ 
\\
\hline

 Water Survival (wk) & $\Wremoval^{-1}$ & $\Sexpr{myround(52 / (h3_mles %>% filter(prof_var == 'mu_B') %>% pull(value)), 3)}$
   &
  $(\Sexpr{myround(1 / (all_ci %>% filter(prof_var == 'mu_B') %>% pull(upper)), 3)}, \Sexpr{myround(1 / (all_ci %>% filter(prof_var == 'mu_B') %>% pull(lower)), 3)})$ 
\\
\hline

 Human to Water Shedding & $\Wshed$ & $\Sexpr{signif(h3_mles %>% filter(prof_var == 'thetaI') %>% pull(value), 3)}$
   &
  $(\Sexpr{signif(all_ci %>% filter(prof_var == 'thetaI') %>% pull(lower), 3)}, \Sexpr{signif(all_ci %>% filter(prof_var == 'thetaI') %>% pull(upper), 3)})$ 
\\
\hline

 Asymptotic Shedding & $\asymptomRelativeShed$ & $\Sexpr{myround((h3_mles %>% filter(prof_var == 'XthetaA') %>% pull(value))^2, 3)}$
   &
