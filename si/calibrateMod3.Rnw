...
bench_diffs <- dplyr::left_join(
  ...
  rename(ll_mod3 = est, ll_bench = ll) %>%
...
The inclusion of these parameters resulted in an overall increase of $\Sexpr{round(max(h3_final_results$search2$logLiks$logLik) - max(haiti3_sub_fit$search2$logLiks$logLik), 1)}$ log-likelihood units.
