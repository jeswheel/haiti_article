...
\title{Supplement to ``{\mytitle}''}
  \author{   Provisional author list\\
 Jesse Wheeler, Anna Rosengart, Zhuoxun Jiang, Kevin Hao En Tan, Noah Treutle, Edward L. Ionides
 \\ \hspace{.2cm}\\
...
The utility of overdispersed models, where the infinitesimal variance of the flow exceeds the infinitesimal mean, has become widely recongized \eic{REFS}.
...
Since rates should be non-negative, Gaussian noise is not appropriate and gamma noise is a convenient option that has found various applications \eic{REFS}.
...
For ODE models, numerical methods are well studied and a standard numerical solution package such as deSolve in R is adequate for our purposes.
...
To address the issue of making our model consistent with total population as a known quantity, based on census data, in practice we use a deterministic specification of $\Delta N_{\bullet j}$ rather than the Poisson form in \eqref{eq:numerics6}.
\eic{CHECK THIS. DO WE HAVE A CONSTANT POPULATION, OR A POPULATION OF DETERMINISTICALLY GROWING SIZE? DO WE USE BIRTH DATA OR TOTAL POPULATION DATA?}
To make birth/immigration events consistent with death/emigration events and population census, we formally model the death rate as a rate of returning to the susceptible class $S$, and use external transitions from $\bullet$ into $S$ to describe only net population increase.
\jwc{CHECK THIS}.
...
The panel iterated filter (PIF) \citep{breto20} has been successfully used to fit parameters $\theta$ of panelPOMPs in several studies \jwc{TODO: REFs}.
...
In the case of a panelPOMP, the parameter vector $\theta$ may be composed of a set of shared parameters $\phi$, and a set of unit-specific parameters $\psi_{\seq{1}{U}}$, for units $\seq{1}{U}$.
Here, we say that a parameter $\psi_{k}$ for $k \in \seq{1}{U}$ is unit-specific if it is not involved in the one-step transition density or the measurement model of any other unit $l \neq k \in \seq{1}{U}$, and shared parameters are those that are not unit-specific.
The PIF algorithm treats the parameter vector $\theta = (\phi, \psi_{\seq{1}{U}})$ as a complete entity and gives no distinction between shared and unit-specific parameters.
...
In this algorithm, shared $\phi$ and unit-specific parameters $\psi_{\seq{1}{U}}$ are handled differently when updating the parameter vector $\theta = (\phi, \psi_{\seq{1}{U}})$.
The BPIF updates $\theta$ by updating the shared vector $\phi$ in the same way that is done with $PIF$, but the indices for weights for unit-specific parameters $\psi_{\seq{1}{U}}$ are only updated using data from the corresponding
unit.
...
\begin{table}
  ...
  \begin{tabular}{|c|c|c|c|c|}\hline
    ...
    Water-to-Human & $\beta_W$ & --- & $\beta_W$ & $\beta$ \\\hline
...
In this article we claimed that we were able to obtained better fits to the observed data using the same models that were proposed in \cite{lee20}.
...
According to the supplement materials, this was because model realizations from a single parameter set retained substantial variability, but multiple realizations from a collection of parameter sets resulted in a reasonable visual fit to the data \jwc{REF???}.
...
This practice is problematic given Figures S8 and S9 of the supplement material \jwc{REF???}, which suggest that some parameter sets that were used for inference were several hundred log-likelihood units lower than the best performing sets of parameters.
...
Fortunately, the range and median values for each of the final model parameters are provided in Tables S10 and S11 of the supplement, and bivariate relationships between some of the fitted parameters are displayed in Figures S8 and S9.
We therefore use these basic summaries to reconstruct an approximation to the set of parameters that were used by the Model~1 authors to perform inference.
...
<<sampleBetaNu, echo=FALSE, cache=TRUE>>=
set.seed(141837)
sample_beta_nu <- function(mu_beta = 5.8, mu_nu = 0.96) {
  ...
  X <- MASS::mvrnorm(n = 300, mu = c(mu_beta, mu_nu), Sigma = cov_mat)
...
<<sample_rho, echo=FALSE, cache=TRUE>>=
sample_rho <- function(beta) {
  ...
  rho_group1 <- rbernoulli(300, probs)
...
<<sample_tau, echo=FALSE, cache=TRUE>>=
sample_tau <- function(mu = 4.1) {
  ...
  X <- rnorm(300, mean = mu, sd = (1.7-mu) / qnorm(0.001))
...
<<initialValueParams, echo=FALSE, cache=TRUE>>=
...
starting_params$E_0 <- sample(1:1752, size = 300, replace = TRUE, prob = E_0_probs) / POP
starting_params$I_0 <- sample(1:2271, size = 300, replace = TRUE, prob = I_0_probs) / POP

starting_params$S_0 <- 1 - starting_params$E_0 - starting_params$I_0
@

<<RemainingParams, echo=FALSE, cache=FALSE>>=
h1_epi <- haiti1()
...
# Local MIF near MLE
stew(
  file = "output/reprod_epi_MIF2.rda",
  {
...
<<PlotEpiDist, echo=FALSE, fig.cap="Compare to Figure S8 of Lee et al. (2020) supplement.", fig.width=5.1, fig.height=5.1, fig.align='center'>>=
epi_results <- as.data.frame(t(coef(MIF2_search)))
epi_results$logLik <- logLik(MIF2_search)
...
epi_complete <- filter(epi_results, logLik > -2200)

# First, we need to simulate the epidemic model using the fitted parameters.
registerDoParallel(20)
registerDoRNG(18599687)
foreach(
  i = 1:nrow(epi_complete),
  .combine = rbind
) %dopar% {
  guess <- unlist(epi_complete[i, ])
  sims <- simulate(h1_epi, nsim = 20, format = 'data.frame',
...
# Local MIF near MLE
stew(
  file = "output/reprod_end_MIF2.rda",
  {
    foreach(
      i = 1:nrow(end_start),
      .packages = c('pomp'),
      ...
      # rinit_parms["S"] <- POP - (rinit_parms["E"] + rinit_parms["I"] + rinit_parms["A"] + rinit_parms["R"])
      ...
      h1_end <- h1_end %>% pomp(
        rinit = new_rinit,
        statenames = c("S", "E", "I", "A", "R", "incid", "foival", "Str0", "Sout", "Sin")
      )
      ...
      coef(h1_end) <- coef(m2)
      sims <- simulate(
        h1_end, nsim = 20, format = 'data.frame',
        params = coef(m2)
      )
      sims$param_set <- i
...
<<plotEndParams, fig.width=5.1, fig.height=5.1, fig.align='center', fig.cap="Bivariate relationships between variables after fitting endemic period. Compare to S9 of Lee et al. (2020) supplement.", cache=TRUE, echo=FALSE>>=
MIF_params <- matrix(nrow = length(MIF2_search_end) / 2, ncol = length(coef(MIF2_search_end[[1]])))
all_sims_end <- data.frame()
liks <- rep(0, nrow(end_start))
for (i in 1:nrow(end_start)) {
  MIF_params[i, ] <- coef(MIF2_search_end[[(2 * i) - 1]])
  all_sims_end <- rbind(all_sims_end, MIF2_search_end[[(2 * i)]])
  liks[i] <- logLik(MIF2_search_end[[(2 * i) - 1]])
}

colnames(MIF_params) <- names(coef(MIF2_search_end[[1]]))
...
<<plotMod1Sims, echo=FALSE, fig.cap="Simulations from various parameter sets. Simulations from parameter sets with likelihood $<$ -2200 are removed.", cache=TRUE, fig.width=4.5, fig.height=4.25>>=
end_complete <- filter(end_results, logLik > -2200)

bad_end_pars <- which(end_results$logLik < -2200)

quants_end <- all_sims_end %>%
...
ggplot() +
  ...
  geom_line(data = quants, aes(x = date, y = q50), col = '#45ac5f') +
  geom_ribbon(data = quants, aes(x = date, ymin = q025, ymax = q975), alpha = 0.5,
              fill = '#45ac5f') +
...
The source code that was used to implement Model~2 in \cite{lee20} was written in the \code{Python} programming language, and is publicly available at \url{https://github.com/lulelita/HaitiCholeraMultiModelingProject}.
In order to perform our analysis in a unified framework, we re-implemented this model in the \code{R} programming language using the \code{spatPomp} package \citep{asfaw21github}.


\subsection{Model~3 Replication}

The code that was originally used to implement Model~3 is archived with the DOI: 10.5281/zenodo.3360723, and also available in the public GitHub repository: \code{jcblemai/haiti-mass-ocv-campaign}.
...
<<cache=T, echo=F, fig.cap="Recreation of model 2 using the pomp package.", fig.height=4>>=
library(haitipkg)
library("pomp")
library("dplyr")
library("grid")
...
# temporary fix. need to figure out why i cant install panelpomp
p2_end <- coef(h2_end)
p2_end[c("VR","Mu","v")] <- c(1.304e-9, 1.897e2, 8.201e1)

traj1 <- trajectory(h2_epi,params=p2_epi,format="data.frame")
traj1$Ctotal <- rowSums(traj1[,paste0("C",1:10)])*0.2
traj2 <- trajectory(h2_end,params=p2_end,format="data.frame")
traj2$Ctotal <- rowSums(traj2[,paste0("C",1:10)])*0.2

traj_agg <- rbind(traj1[,c("year","Ctotal")], traj2[,c("year","Ctotal")])
names(traj_agg) <- c("year","cases")
traj_agg$group <- "trajectory"

haiti_agg <- aggregate(h2_data$cases,
                       by = list(h2_data$year),
                       FUN = sum)
names(haiti_agg) <- c("year","cases")
haiti_agg$group <- "reported data"

final <- rbind(traj_agg, haiti_agg)

pp1 <- ggplot(data=final %>% filter(year<2014.161),
             aes(x=year,y=cases,group=group)) +
  geom_line(aes(linetype=group,color=group)) +
  scale_linetype_manual(values=c("dashed","solid")) +
  scale_x_continuous(breaks=seq(2010,2020,by=1)) +
  theme(legend.position=c(0.7,0.7))

pp2 <- ggplot(data=final %>% filter(year>2014.161),
             aes(x=year,y=cases,group=group)) +
  geom_line(aes(linetype=group,color=group)) +
  scale_linetype_manual(values=c("dashed","solid")) +
  scale_x_continuous(breaks=seq(2010,2020,by=1)) +
...
<<cache=T, echo=F, fig.cap="Endemic trajectory with state values carried over from epidemic model.", fig.height=2>>=
library("subplex")
theme_set(theme_bw())

h2_end_joint <- haiti2(region="after",joint=TRUE) %>% pomp()
sim <- trajectory(h2_epi,params=p2_epi,format="data.frame")
end_IVPS <- sim[nrow(sim),-c(321,322)] %>% as.numeric()
names(end_IVPS) <- names(h2_end_joint@params[24:343])

joint_params <- c(p2_end[1:23],end_IVPS)
ofun <- traj_objfun(h2_end_joint,est=c("Mu","v","VR"),
                     params=joint_params)
theta <- c(Mu=1e3,v=100,VR=2e-11)
fit <- subplex(par=log(theta), fn=ofun)
joint_params[c("Mu","v","VR")] <- exp(fit$par)

traj3 <- trajectory(h2_end_joint,params=joint_params, format="data.frame")
traj3$Ctotal <- rowSums(traj3[,paste0("C",1:10)])*joint_params["Rho"]

traj_agg <- rbind(traj3[,c("year","Ctotal")])
names(traj_agg) <- c("year","cases")
traj_agg$group <- "trajectory"
final <- rbind(traj_agg, haiti_agg)

pp3 <- ggplot(data=final %>% filter(year>2014.161),
             aes(x=year,y=cases,group=group)) +
  geom_line(aes(linetype=group,color=group)) +
  scale_linetype_manual(values=c("dashed","solid")) +
  scale_x_continuous(breaks=seq(2010,2020,by=1)) +
