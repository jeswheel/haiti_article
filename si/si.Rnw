...
\begin{equation}
  \label{model1:meas}
  Y_{n} \mid \Delta N_{E_{\vaccCounter}I_{\vaccCounter}}(n) = z(n) \sim \text{NB}\left(\reportRate z(n), \obsOverdispersion \right),
\end{equation}
where $Y_n$ is the reported cholera cases at time $n \in 1:N$ and $\Delta N_{E_{\vaccCounter}I_{\vaccCounter}}(n)$ is the number of individuals who moved from compartment $E_\vaccCounter$ to $I_\vaccCounter$ since observation $n-1 \in 0:N-1$, and $\text{NB}\left(\reportRate z, \obsOverdispersion \right)$ is the negative binomial distribution with mean $\reportRate z$ and variance $\reportRate z+ \psi^2\reportRate^2 z^2)$. \jwc{CHECK THIS}.
...
\begin{equation}
  \label{model2:meas}
  \log\big(Y_{u, n} + 1\big) \mid \bm{X}^{(2)}(n) = \bm{x}^{(2)}(n) \sim \text{N}\left(\log \big(\reportRate \sum_{\vaccCounter} I_{u, \vaccCounter}(n) + 1\big), \obsOverdispersion^2 \right)
\end{equation}

Where $\sum_{\vaccCounter} I_{u, \vaccCounter}(n)$ is the total number of individuals who are infected at time $n$ in unit $u \in 1:U$, for all vaccination campaigns $\vaccCounter$.
We note that fitting a model with this loss function is equivalent to fitting using least squares, with $\log(y_{u, n} + 1)$ as the response variable.
