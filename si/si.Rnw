...
Despite having source code available, no point estimates for model parameters were provided by \cite{lee20}.
...
When accounting for parameter uncertainty, one should instead weigh resulting model projections based on the likelihood of the set of parameters that were used to obtain the model realizations, as was done by \cite{king15}.
...
N_LEE_SIMS <- switch(run_level,  10,  15,   20)
...
foreach(
  ...
  guess <- lee1_epi_mif$lee1_epi_mif_params[i, ]
...
lee1_epi_evals <- readRDS(file = paste0("../model1/", RL_dir, "lee1_epi_evals.rds"))

epi_results <- as.data.frame(lee1_epi_mif$lee1_epi_mif_params)
epi_results$logLik <- lee1_epi_evals$pfLL

GGally::ggpairs(
  dplyr::filter(epi_results, logLik > -20000),
...
To do this, we use the same number of particles and MIF iterations that were used by \citet{lee20}.
...
foreach(
  i = 1:nrow(lee1_end_mif),
  .combine = rbind
) %dopar% {
  r_params <- lee1_end_mif[i, ]
...
lee1_end_evals <- readRDS(file = paste0("../model1/", RL_dir, "lee1_end_evals.rds"))

end_results <- as.data.frame(lee1_end_mif)
end_results$logLik <- lee1_epi_evals$pfLL

GGally::ggpairs(
  filter(end_results, logLik > -20000),
  columns = c("rho", "tau", "beta1", "nu", "logLik")
)
@
...
While not a perfect replication of the results given by \citet{lee20sup}, the similarities between the model simulations, and the fact that we based our implementation of Model~1 primarily on the source code that was provided, makes us confident that we have been able to obtain a reliable representation of the model described by \citet{lee20}.
...
epi_complete <- filter(epi_results, logLik > -20000)
bad_epi_pars <- which(epi_results$logLik < -20000)

quants <- all_sims %>%
  filter(!param_set %in% bad_epi_pars) %>%
  group_by(week) %>%
  summarize(
...
end_complete <- filter(end_results, logLik > -20000)
bad_end_pars <- which(end_results$logLik < -20000)
...
NSIM <- switch(run_level, 50, 100, 1000)
...
lee3 <- haiti3_lee_et_al()
lee3_sims <- simulate(lee3, nsim = NSIM, seed = 321, format = 'data.frame')
