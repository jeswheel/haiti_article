...
\begin{equation}
  \label{model1:meas}
  Y_{n} \mid \Delta N_{EI}(n) = \reportChange \sim \text{NB}\left(\reportRate \reportChange, \obsOverdispersion \right),
\end{equation}
where $Y_n$ is the reported cholera cases at time $t_n \in t_1:t_N$ and $\Delta N_{EI}(n)$ is the sum total of individuals across each vaccination compartment $\vaccCounter \in 1:\vaccClass$ who moved from compartment $E_\vaccCounter$ to $I_\vaccCounter$ since observation $t_{n-1}$, and $\text{NB}\left(\reportRate \reportChange, \obsOverdispersion \right)$ is the negative binomial distribution with mean $\reportRate \reportChange$ and variance $\reportRate \reportChange \big(1 + \frac{\reportRate \reportChange}{\obsOverdispersion}\big)$.
...
\begin{equation}
  \label{model2:meas}
  \log\big(Y_{u, n} + 1\big) \mid \Delta N_{EI}(n) = \reportChange \sim \text{N}\left(\log \big(\reportRate \reportChange + 1\big), \obsOverdispersion^2 \right),
\end{equation}
where $\Delta N_{EI}(n)$ is the sum total of individuals across vaccination compartment $\vaccCounter \in 0:4$ and unit $u$ who moved from compartment $E_{u\vaccCounter}$ to $I_{u\vaccCounter}$ since observation $t_{n-1}$.
...
\begin{equation}
  ...
  Y_{u, n} \mid \Delta N_{EI}(n) = \reportChange \sim \text{N}\left(\reportRate \reportChange, \obsOverdispersion^2 \right),
\end{equation}
...
\begin{equation}
  ...
  Y_{u, n} \mid \Delta N_{EI}(n) = \reportChange_1, \Delta N_{EA}(n) = \reportChange_2 \sim \text{N}\left(\reportRate \big(\reportChange_1 + \reportChange_2\big), \obsOverdispersion^2 \right),
\end{equation}
...
\begin{equation}
  \label{model3:meas}
  Y_{u, n} \mid \Delta N_{S_{u \vaccCounter}I_{u \vaccCounter}}(t) = \reportChange_u \sim \text{NB}\left(\reportRate \reportChange_{u}, \obsOverdispersion \right),
\end{equation}
where $\Delta N_{S_{u \vaccCounter}I_{u \vaccCounter}}(n)$ is the number of individuals who moved from compartment $S_{u \vaccCounter}$ to $I_{u \vaccCounter}$ since observation $t_{n-1}$.
...
The latent states of this model are initialized by estimating of the number of infected, asymptomatic, and recovered individuals at time $t_0$ by using observed case counts at time $t_1$.
...
# This function sets the rw_sd for named parameters, first search.
set_rw_1 <- function(x) {
...
# This function sets the rw_sd for named parameters, second search.
set_rw_2 <- function(x) {
  if (gsub("[[:digit:]]+$", "", x) %in% shared_param_names) {
    return(0.003)
  } else if (x %in% paste0("k", 1:10)) {
...
# This function sets the rw_sd for named parameters, final search.
set_rw_3 <- function(x) {
  if (gsub("[[:digit:]]+$", "", x) %in% shared_param_names) {
    return(0.0025)
  } else if (x %in% paste0("k", 1:10)) {
...
# TODO: Remove SEARCH3 when re-running everything.
...
In particular, we discuss why we decided to include additional model parameters---those that are associated with the initial conditions of the model and the behavior of the system during Hurricane Matthew---that were not considered by \cite{lee20}.
To calibrate this model, we used the iterated block particle filter (IBPF) method of \cite{ionides22}.
Due to the novelty of this algorithm, there does not exist published examples the IBPF algorithm used for data analysis outside of the papers in which the algorithm was introduced \citep{ning21ibpf,ionides22}, which at least partially motivates the inclusion additional details in this section.
...
A subsequent refinement search used starting values corresponding to the largest model likelihoods obtained in the previous round.
...
The maximum likelihood we obtained with the model was $\Sexpr{round(max(haiti3_orig_fit$search2$logLiks$logLik))}$, which is higher than the log-ARMA benchmark ($\Sexpr{round(sum(unitLikesARMA$ll))}$). % TODO: Check this, numbers don't match up.
...
Whatever the case may be, the inclusion of the seven parameters resulted in a $\Sexpr{round(max(h3_final_results$search2$logLiks$logLik) - max(haiti3_orig_fit$search2$logLiks$logLik), 1)}$ increase in log-likelihood, which is a highly statistically significant difference.
...
Along with visual comparisons to the data, this claim was supported by comparing likelihoods and AIC values in Table~2 in the manuscript. % TODO: Double check the table number matches
