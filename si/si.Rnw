...
Our treatment follows \citet{breto09} \jwc{TODO: Cite the short course}.
...
A flow into compartment $i$ from outside the system is denoted by $N_{\circ i}$, and a flow out of the system from compartment $i$ is denoted by $N_{i\circ}$.
We call $\circ$ a source/sink compartment, though it is an irregular compartment since $X_{circ}(t)$ is not defined.
...
For the case $i=\bullet$, the treatment used by Model~1 is to set
\begin{equation}
\label{eq:numerics6}
...
Models~2 and 3 used an alternative approach, balancing the total number of flows in and out of the compartment, i.e., $\sum_{i}N_{\bullet i}(t) = \sum_{i}N_{i \bullet}(t)$, in order to make the model consistent with the known total population.
In this case, we formally model the death rate as a rate of returning to the susceptible class $S$, and use external transitions from $\bullet$ into $S$ to describe only net population increase.
...
\begin{equation}
  ...
  Y_{u, n} \mid \Delta N_{EI}(n) = z_1(n), \Delta N_{EA}(n) = z_2(n) \sim \text{N}\left(\reportRate \big(z_1(n) + z_2(n)\big), \obsOverdispersion^2 \right),
\end{equation}
...
To perform inference on POMP models, it is necessary to propose an initial probability density for the latent process $f_{X_0}(x_0;\theta)$, including the possibility that the initial values could be known, or could be a non-random function of the unknown parameter vector, $\theta$.
...
As described in the main test, the latent states are initialized by estimating of the number of infected, asymptomatic, and recovered individuals at time $t_0$ by using observed case counts at time $t_1$.
The initial value for the bacteria compartment is then determined using the rainfall dynamics of Eq.~23.
For the departments of Grande'Anse, Nord-Ouest, and Sud-Est, estimating the latent states at time $t_0$ using the observed data at time $t_1$ is problematic (see Sec.~\ref{sec:mod3Cal}), so we instead estimate initial value of the latent state $I_{u0}(t_0)$ for the integers $u \in \{3, 7, 9\}$, which correspond to the aforementioned departments.
The remaining hidden states are estimated by enforcing model dynamics using this result.
...
# Vector containing all of the estimate parameter names.
est_param_names <- c(
...
# This function sets the rw.sd for named parameters, first search.
set_rw_1 <- function(x) {
...
# This function sets the rw.sd for named parameters, second search.
set_rw_2 <- function(x) {
  if (gsub("[[:digit:]]+$", "", x) %in% shared_param_names) {
    return(0.003)
  } else if (x %in% paste0("k", 1:10)) {
...
# This function sets the rw.sd for named parameters, final search.
set_rw_3 <- function(x) {
  if (gsub("[[:digit:]]+$", "", x) %in% shared_param_names) {
    return(0.0025)
  } else if (x %in% paste0("k", 1:10)) {
...
save_file <- "../model3/si/fit_orig_model.rds"
...
# Create dataset that will be used to fit ARMA models. To do this, we
# need to make sure that missing values are marked as NA, so we check
# for any jumps in time.
...
# If there is a jump, insert NA between observations
temp <- MODEL3_CASES
...
mod3_cases <- temp %>% dplyr::select(-time_diff)
...
# log-likelihoods.
bpf_orig3_out <- bake(
  file = "../model3/si/bpf_orig3_cond_LL.rds", {
...
This result led us to include parameters $\Whur{u}$ and $\hHur{u}$ in Eq.~23 of the main text, which allows for an increase in the transmission rate between environmental cholera and humans for Sud and Grand'Anse during and after the hurricane.
...
t(bpf_orig3_out$block.cond.loglik) %>%
  as.data.frame() %>%
  mutate(t = bpf_orig3_out$times) %>%
  pivot_longer(
    cols = -t,
...
This may be particularly true at the onset of an infectious disease outbreak, as lower public awareness and inadequate testing infrastructure may result in many cases going undetected.
...
Whatever the case may be, the inclusion of the seven parameters resulted in a $\Sexpr{round(max(h3_final_results$search3$logLiks$logLik) - max(haiti3_orig_fit$search3$logLiks$logLik), 1)}$ increase in log-likelihood, which is a highly statistically significant difference.
...
This reproduction allows us to estimate the likelihoods of the \cite{lee20} version of Models~1--3, and also provides a demonstration of reproducibility.
...
In this case, these discrepancies may be attributed to implementing the model in two different programing languages.
...
For example, the parameters $\transmission$, $\Wbeta{}$ (See Table~\ref{tab:translate}) had reported values of $9.9 \times 10^{-7}$ and $4.03 \times 10^{-2}$, respectively (Table~S13 of \cite{lee20sup}), but were actually fit to data and therefore likely had values that were far more precise.
...
This results in a natural set of parameters near the MLE that could be used in in the discrete prior distribution.
