...
One concern that arises when inspecting the files \code{fitInPieces3paramsCleanMay2019Public.py} and \code{fitInPiecesMuWithFracSusFixedAllInfectionsPublic.py} is that the output of these files (which contains the estimated parameters) was not referenced/used in either the file that generates the sensitivity analysis or the file that generates the model simulations; instead, alternative (non-extant) files with similar names are loaded, suggesting parameters that were used to generate the final results were not a direct result of the estimated parameters.
Furthermore, inspection of the script \code{simulateHaitiFullTimeInPiecesAllVaccinationScenariosSingleParameterVectorAllInfectionsPublic.py}---which is used to obtain simulations for each of the vaccination scenarios---results in a number of concerns.
First, the script uses parameters from the pickle file \code{leastSquaresDataUpTo2019/fitEpidemicPiece\_resultsMatrix\_18Mar2019.pickle}, which only appears in the repository as a commented out line of code in the file \code{generateSetsOfParametersForSensitivityAnalysisPublic.py}, suggesting that the set of parameters used to simulate each vaccination scenario was not the most recent set of parameters generated by the sensitivity analysis.
Second, the script appears to use the set of differential equations from the function \code{choleraEqs11WithVaccinationNetwork} for the epidemic phase, even though parameter values for this phase were estimated using \code{choleraEqs10WithVaccinationNetwork}.
