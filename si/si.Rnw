...
The maximum likelihood we obtained with the model was $\Sexpr{round(max(haiti3_sub_fit$search2$logLiks$logLik))}$, which is higher than the benchmark model ($\Sexpr{round(sum(unitLikes_bench$ll))}$).
...
@
\caption{\label{fig:h3UnitLikes}Log-likelihoods of Model~3 for each department compared to the corresponding benchmark model prior to the inclusion of parameters for initial values and Hurricane Matthew.}
\end{figure}
...
When compared to a benchmark, this level of detail can provide useful information about which observations are well described by the model and which are poorly described.
In Fig.~\ref{fig:condLL}, we plot the conditional log-likelihoods of each observation.
...
ggplot(bench_M3_joined, aes(x = ll, y = reorder(unit, ll))) +
  ...
  theme(legend.title = element_blank(),
        axis.title.y = element_blank())
@
\caption{\label{fig:finalUnitLL}Log-likelihoods of Model~3 for each department compared to the corresponding benchmark model after adding and estimating parameters related to initial values and Hurricane Matthew.}
\end{figure}
...
Whatever the case may be, the inclusion of the seven parameters resulted in a $\Sexpr{round(max(h3_final_results$search2$logLiks$logLik) - max(haiti3_sub_fit$search2$logLiks$logLik), 1)}$ increase in log-likelihood, which is a highly statistically significant difference.
