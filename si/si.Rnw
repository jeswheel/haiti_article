...
Two subsequent refinement searches used starting values corresponding to the largest model likelihoods obtained in the previous round.
This is similar to the approach used by \cite{ionides22}. 
...
In order to perform our analysis in a unified framework, we re-implemented this model in the \code{R} programming language using the \code{spatPomp} package \citep{asfaw22cran}, which facilitates the creation of meta-population models.
