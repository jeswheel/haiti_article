...
\eic{CHECK: EVERYTHING IN SI SHOULD BE REFERENCED IN THE MAIN TEXT.}
...
\end{equation}
\eic{medskip IS VERTICAL SPACE. I USUALLY USE hspace}
...
\eic{HERE AND ELSEWHERE, in Lee et al SHOULD BE by Lee et al. CONVENTIONALLY, THE TEXT CITATION REFERS TO THE PEOPLE RATHER THAN THE ARTICLE.}
...
\end{equation}
where $\xi_u \in (0, 1)$ is an initial value parameter that allows some flexibility in determining the initial state of the bacteria compartment.
...
\begin{equation}
  \label{model1:meas}
  y_{t} \mid \Delta N_{E_{\vaccCounter}I_{\vaccCounter}}(t) = z(t) \sim \text{NB}\left(\reportRate z_(t), \obsOverdispersion \right)
\end{equation}

Where $y_t$ is the reported cholera cases at time $t \in 1:N$ and $\Delta N_{E_{\vaccCounter}I_{\vaccCounter}}(t)$ is the number of individuals who moved from compartment $E_\vaccCounter$ to $I_\vaccCounter$ since observation $t-1 \in 0:N-1$.
\eic{HERE AND ELSEHWERE: PUNCTUATION. NO NEED FOR A NEW PARAGRAPH HERE AFTER THE DISPLAY EQUATION. ALSO, PROBABLY THERE SHOULD BE A COMMA AT THE END OF THE EQUATION AND Where THEN IS NOT CAPITALIZED}

\eic{AS A PEDANTIC POINT WHICH IS NOT UNIVERSALLY IMPLEMENTED BUT I HAPPEN TO THINK IS WORTHWHILE: THE NOTATION IN S16 USUALLY CORRESPONDS TO A DISTRIBUTION, BUT $y_n$ (WHICH I SUPPOSE HERE IS WRITTEN $y_t$) IS DATA, AND THEREFORE A NUMBER NOT A RANDOM VARIABLE. WE MODEL IT BY A RANDOM VARIABLE $Y_n$. THE DISTINCTION MAY NOT BE CONFUSING FOR MANY, BUT WHEN WE ARE ENCOURAGING OURSELVES AND OTHERS TO THINK ABOUT THE DIFFERENCES BETWEEN DATA AND MODELS FOR DATA, IT IS COUNTER-PRODUCTIVE TO HAVE NOTATION WHICH SUPPRESSES THIS DIFFERENCE.}

\eic{DUE TO DIFFERENT PARAMETERIZATIONS OF NB (AS WELL AS TO CLARIFY THE MODEL) I THINK IT IS GOOD PRACTICE TO GIVE THE MOMENTS, E.G., ``, and $\text{NB}\left(\reportRate z, \obsOverdispersion \right)$ is the negative binomial distribution with mean $\reportRate z$ and variance $\reportRate z+ \psi^2\reportRate^2 z^2)$'' IF THIS IS CORRECT.}

\subsection{Model~2}

As mentioned in the main text, Model~2 was fit using log-transformed observed cases.
...
\begin{equation}
  \label{model2:meas}
  \log\big(y_{u, t} + 1\big) \mid \bm{X}^{(2)}(t) = \bm{x}^{(2)}(t) \sim \text{N}\left(\log \big(\reportRate \sum_{\vaccCounter} I_{u, \vaccCounter}(t) + 1\big), \obsOverdispersion^2 \right)
\end{equation}

Where $\sum_{\vaccCounter} I_{u, \vaccCounter}(t)$ is the total number of individuals who are infected at time $t$ in unit $u \in 1:U$, for all vaccination campaigns $\vaccCounter$.
We note that fitting a model with this loss function is equivalent to fitting using least squares, with $\log(y_{u, t} + 1)$ as the response variable.
...
\begin{equation}
  \label{model3:meas}
  y_{u, t} \mid \Delta N_{S_{u \vaccCounter}I_{u \vaccCounter}}(t) = z_{u}(t) \sim \text{NB}\left(\reportRate z_{u}(t), \obsOverdispersion \right)
\end{equation}

Where $\Delta N_{S_{u \vaccCounter}I_{u \vaccCounter}}(t)$ is the number of individuals who moved from compartment $S_{u \vaccCounter}$ to $I_{u \vaccCounter}$ since observation $t-1 \in 0:N-1$.
...
\eic{USUALLY BEST TO AVOID believe IN SCIENTIFIC WRITING. CAN WE SAY SOMETHING SOLID ABOUT WHAT WE HAVE FOUND. EVEN, I THINK THE FOLLOWING IS TRUE: We have found that this modification leads to improved likelihood maximization performance, especially as the number of units increases.}
