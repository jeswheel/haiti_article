...
\section{Replication of Lee et al.~(2020)}
...
<<plotEndParams, fig.width=7, fig.height=7, fig.align='center', fig.cap="Bivariate distributions of parameter estimates after fitting endemic phase of the Model~1 following the procedure described by Lee et al. (2020). Compare to Figure S9 in the supplement of Lee et al. (2020).", cache=FALSE, echo=FALSE,warning=FALSE>>=
GGally::ggpairs(
  h1LeeStartsEnd,
  columns = c("rho", "tau", "beta1", "nu", "loglik")
)
...
We note that the travel and water matrices used to implement the complex dynamics in Model~2 \cite{lee20sup} are not available in either the Zenodo archive or the GitHub repository;
instead, we obtained these matrices via personal correspondence with the Model~2 authors.
Using these matrices, and the point estimates for model parameters provided by \cite{lee20sup}, we created trajectories of the cholera dynamics and compared this to available data.
...
Another potential explanation for the discrepancy is that the parameters that we used are only approximately the same as those used by Lee et al. (2020) \cite{lee20sup}.
...
Details about the measurement models and how latent states were initialized for the epidemic model were not provided by Lee et al. (2020) \cite{lee20sup} and therefore these details must be inferred by looking at the provided source code.
