# Fitting profiles for Haiti 3: refitting profiles while fixing std_W at its 
# marginal MLE. 
#
# ********
# NOTE:
...
#   the RUN_LEVEL == 3 version of this script took 12911 compute hours. Partial
...
# ********
#
# This script must be ran after the results of H3profile.R have been obtained. 
# In the first H3profile, we found that the profile for the parameter std_W 
# was noticeably higher (measured by log-likelihood) than the profiles for 
# parameters. Because the maximum of the true profile necessarily should be 
# on the same scale for each parameter, this indicated that the parameter 
# search does best when the value of std_W is fixed. Therefore we re-compute 
# many of the same profiles as before while fixing std_W at it's marginal 
# MLE. 
#
...
reg <- makeExperimentRegistry(
  file.dir = paste0('model3/profile_non_std_W_RL', RUN_LEVEL),
  seed = 739164,
...
for (pp in prof_params) {
  ...
  if (pp == "mu_B") {
    prof_values <- seq(325, 700, length.out = 25)
  } else if (pp == 'XthetaA') {
    prof_values <- seq(1e-8, sqrt(0.36), length.out = 20)^2
  } else if (pp == 'thetaI') {
    prof_values <- seq(3e-05, 1.25e-04, length.out = 25)
  } else if (pp == 'lambdaR') {
    prof_values <- seq(1e-8, 3, length.out = 25)
  } else if (pp == 'r') {
    prof_values <- seq(1e-5, 2.5, length.out = 25)
  } else if (pp == 'std_W') {  # This condition should never be met, but kept as reference. 
    prof_values <- std_W
  } else if (pp == 'epsilon') {
    prof_values <- seq(0.715^2, 1-1e-5, length.out = 20)^(1/2)
  } else if (pp == 'k') {
    prof_values <- seq(20, 150, length.out = 25)
  }
  ...
  all_params[paste0('std_W', 1:10)] <- std_W  # Fix std_W at the marginal MLE
...
# When we ran the code, the longest running job took 5.24 hours, and the 
# average job took 3.91 hours. There are 3300 total jobs. 
...
# reg = loadRegistry(paste0('model3/profile_non_std_W_RL', RUN_LEVEL))
# getStatus()
#
...
# saveRDS(
#   h3_results,
#   paste0('model3/run_level_', RUN_LEVEL, '/', 'h3_profiles2.rds')
# )
#
