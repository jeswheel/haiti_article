...
This permits the consideration of demographic stochasticity, which is dominant for small infected populations, togehter with log scale stochasticity (also called multiplicative, or environmental, or extra-demographic) which is dominant at high population counts.
...
The decision to model a dynamic system via metapopulation models versus a model aggregated to a larger spatial scale is one of great scientific interest.
...
Parameters that must be fit in Model~3 are shared between each department, the exception to this being the parameters $\beta_{W_u}$, and $\beta_u$, which are unique for each department $u \in \seq{1}{10}$.
To avoid the parameter estimation issue in high-dimensional models, \citet{lee20} simplified the problem by using the MIF2 algorithm to separately estimate parameters for independent department-level models;
...
These summary statistics are sometimes called diagnostic probes \citep{king16}.
