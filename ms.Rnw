...
In the frequentest case, one approach is a quasi-empirical Bayes: endow to set $\Theta_K = \{\theta_1, \theta_2, \ldots, \theta_K\}$ with a discrete uniform distribution, such that $P\big(\paramVec = \theta_k\big) = \frac{1}{K}$ for all values $k \in \seq{1}{K}$.
...
Perhaps the best interpretation of this results, then, is that the model most adequately describes the observed data by having a steady decrease in the number of susceptible individuals.
In this case, the fact that the parameter estimate $\muRS$ is inconsistent with the expected result can be taken as evidence of model mispecification.
