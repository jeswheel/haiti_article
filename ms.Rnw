...
\hline
Number of &
  $\Sexpr{as.character(mod1_n_params)}$ & $\Sexpr{as.character(mod2_n_params)}$ & \multirow{2}{*}{$\Sexpr{as.character(mod3_n_params)}$} & $\Sexpr{mod3_spatPomp_n_params}$ \\
 Fit Parameters & (\Sexpr{as.character(mod1_lee_n_params)}) & ($\Sexpr{as.character(h2_fit$leeFit_n_params)}$) && ($\Sexpr{as.character(lee3_results$n_params, 1)}$)
\\
\hline
\multirow{2}{*}{AIC} &
...
Obtaining forecasts in this fashion allows us to get results that are consistent with the model assumptions and the observed data.
