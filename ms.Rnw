...
\ei{I'VE RE-WRITTEN HERE, LOOKING TO BE MORE SPECIFIC ABOUT THE GOALS AND CONTRIBUTION OF THE PAPER.}
...
These modeling strategies were selected by \cite{li20} to represent the range of approaches used in the research community.
...
The data that were used for this study, and that is the subject of our reanalysis, are displayed in Fig.~\ref{fig:CholeraData}.
...
%% IT SEEMS THIS PARAGRAPH IS DEPRECATED IN FAVOR OF CITING THESE REFERENCES WEHRE APPROPRIAT
%% While this work is focused on the models proposed by Lee et al.~\cite{lee20}, our suggestions have broader relevance.
%% To investigate the extent to which \cite{lee20} is typical of the substantial body of work on Haiti cholera dynamics, we performed a literature review by searching PubMed with keywords: Haiti, cholera, model.
...
\raggedright Observation variance
  ...
  & \multirow{2}{*}{(S15)}
  & \multirow{2}{*}{$\obsOverdispersion = \Sexpr{signif(h2_fit$h2_params['sigma'], 3)}$}
  & \multirow{2}{*}{(S16)}
...
In this article we follow \cite{lee20} by calibrating the parameters of each of our models using maximizing the likelihood, as described in Eq.~\myeqref{eq:likedef}.
...
The main alternative to maximum likelihood estimation is Bayesian inference via Markov chain Monte Carlo, used to analyze the Haiti cholera epidemic by \cite{andrews11,rinaldo12,lewnard16,trevisin22,sallah17,azman12,kuhn14,mari15,gatto12}.
...
The proposal of a linear trend in transmission is a result of observing an apparent decrease in reported cholera infections from 2012-2019 in Fig.~\ref{fig:CholeraData}.
