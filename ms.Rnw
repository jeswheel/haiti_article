...
Parameters that must be fit in Model~3 are shared between each department, the exception to this being the parameters $\beta_{W_u}$, and $\beta_u$, which take department-specific values for each $u \in \seq{1}{10}$.
...
Fitting Model~3 in this fashion simplifies the problem of parameter estimation, but it also introduces additional technicalities that must be addressed \eic{THIS MAKES IT SEEM LIKE THE FITTING METHOD CAUSES THE PROBLEM, BUT THE LEE ET AL APPROACH ALSO HAS THIS ISSUE. ARGUABLY, IT IS A PART OF THE MODEL NOT THE METHOD, THOUGH THE TWO ARE RELATED. MODEL 3 IS WRITTEN AS FULLY COUPLED IN SEC 2.3, SO FITTING THE NON-COUPLED VERSION IS ARGUABLY FITTING SOMETHING ELSE ENTIRELY, THOUGH IT CAN ALWAYS BE VIEWED AS AN APPROXIMATION.}
