...
\cite{lee20} included process noise in Model~3 but not in Model~1, i.e., they set $\sigmaProc^2 = 0$.
...
\text{human}\end{array}$
  ...
  & \eqref{model1:lambda}
  & $\transmission=$\Sexpr{signif(h2_fit$epi_params["Beta"],3)}
  & \eqref{model2:lambda}
  & $\begin{array}{l}
...
\end{array}$
  ...
  & $\begin{array}{l}
    ...
    \\
    \beta_W= \Sexpr{signif(h2_fit$epi_params["BetaW"],3)}
    \end{array}$
  & \eqref{model2:lambda}
  & $\begin{array}{l}
...
\text{water}\end{array}$
  & \multicolumn{1}{c}{\hspace{16pt}\missing}
  &
  & $\Wshed = $ \Sexpr{signif(h2_fit$epi_params["Mu"],3)}
  & \eqref{model2:to_W}
...
\end{array}$
  & \multicolumn{1}{c}{\hspace{16pt}\missing}
  &
  & $\Wremoval^{-1} = $ \Sexpr{signif(52/h2_fit$epi_params["Delta"],3)}
  & \eqref{model2:from_W}
