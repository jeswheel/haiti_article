...
This results in confounded mechanisms, as likelihood-based comparisons of a model that permits a change in reporting rate against a model that permits a change in transmission rate may not yield definitive results. In such a case, t}\old{T}he robust statistical conclusion is that a model which allows for change fits better than one which does not\old{---we argue that a decreasing transmission rate is a plausible way to explain this}, but the incidence data themselves \old{do} \new{may} not provide enough information to pin down the mechanism.
...
\TODO{The profile confidence interval for the parameter suggests that the true estimate should be zero, but it was unable to get this value due to the parameter transformation. Should we manually set the value to zero? This result will also affect this paragraph, but only in ways that we already anticipated with the estimate close to zero.}
