...
\new{This framework promotes scientific discovery and supports model-based policy recommendations by simplifying the process for researchers to build off of previous results.}
...
\old{For a mechanism, the model feature could in principle explain the data by helping to account for some different unmodeled phenomenon.}
...
Under the framework of Model~2, the best estimate for this parameter is $\Sexpr{signif(1/h2_fit$h2_params["sigma"],3)} \, \mathrm{yr}$, suggesting that individuals have \old{effectively} permanent immunity to cholera once infected.
