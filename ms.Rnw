...
# 
#                    RUN_LEVEL =  1,   2,    3
N_PARAMS   <- switch(RUN_LEVEL,  10,  20,  300)  # Number of parameters to sample
N_LEE_SIMS <- switch(RUN_LEVEL,  10,  20,   20)  # Number of simulations per parameter
NMIF       <- switch(RUN_LEVEL,   5,  20,   50)  # Number of MIF iterations
NP_MIF     <- switch(RUN_LEVEL,  20,  50,  100)  # Number of particles for IF2. This is too few for proper optimization, but matches Lee et al
...
# Get starting parameters close to those that are provided
# in Lee et al (2020)
starting_params <- get_h1_startVals(N_PARAMS)

# Create epi model for haiti1
h1_epi <- haiti1()

# Do a small global search for starting parameters for beta2-beta6
bounds <- tribble(
  ~param, ~lower, ~upper,
  "beta2", 1e-08, 10,
  "beta3", 1e-08, 10,
...
# Get a sequence of starting values for the betas
betas <- sobol_design(
  lower = lower,
  upper = upper,
  nseq  = N_PARAMS
...
# Combine betas with the starting_params data.frame object.
epi_params <- cbind(starting_params, betas)

# Get the names of all of the remaining coefficients (fixed parameters).
not_in_start_params <- names(coef(h1_epi))[!names(coef(h1_epi)) %in% colnames(epi_params)]

# Add fixed parameters to the matrix of starting parameters
for (i in 1:length(not_in_start_params)) {
  epi_params[, not_in_start_params[i]] <- coef(h1_epi)[not_in_start_params[i]]
}

# The process model of Lee et al (2020) didn't include over-dispersion
epi_params$sig_sq <- 0

# re-ordering starting parameters to match model order
epi_params <- epi_params[, names(coef(h1_epi))]

# Setting the random walk standard deviation. 
epi_rw <- rw.sd(
...
if (RUN_LEVEL != 3 & file.exists("model1/run_level_3/lee1_epi_fit.rds")) {
  ...
  lee1_epi_mif <- bake(
    file = paste0("model1/", rl_dir, "lee1_epi_fit.rds"),
    {
      ...
      # Loop through each of the starting parameters
      foreach(
        i = 1:nrow(epi_params),
        .packages = c('pomp'),
        .combine = c
        ...
        # Get the ith set of starting parameters
        r_params <- unlist(epi_params[i, ])

        # Set the coefficients of model 1 to the ith starting values
        coef(h1_epi) <- r_params

        # Perform short IF2 from the starting value
        mif2(
          h1_epi,
          Np = NP_MIF,
          Nmif = NMIF,
      ...
      } -> lee1_epi_mif_out  # Contains nrow(epi_params) mif2d objects

      # For each output of the MIF2 objects, simulate Model 1 with the IF2
      # parameters and save the states from the final week of the simulation
      foreach(i = 1:length(lee1_epi_mif_out), .combine = rbind) %do% {

        # Get IF2 parameter set
        guess <- coef(lee1_epi_mif_out[[i]])

        # Simulate with the parameter set
        sims <- simulate(
          h1_epi,
          n = N_LEE_SIMS,
          format = 'data.frame',
          ...
          dplyr::filter(week == max(week)) %>%  # Just keep final states, to use later.
...
# Some clean-up of unused variables, to avoid "Environment clutter".
rm(epi_rw, epi_params, not_in_start_params,
   lower, upper, betas, bounds, starting_params, i)
gc()

# Check if the run_level_3 results exist. If they do, load these results. 
# If not, compute the run_level_<RUN_LEVEL> results and save them. 
...
# Our parameter values are now very similar to those used by Lee et al (2020). 
# We therefore use the same rw.sd that was used in their source code. 
end_rw <- rw.sd(
  beta1 = 0.05,
  tau   = 0.005,
  nu    = 0.04,
...
registerDoRNG(21489587)  # For reproducible computing

# Store the parameter values from the end of the IF2 search for the epidemic 
# model as the start values for the IF2 search for the endemic model, as was 
# described by Lee et al (2020)
...
if (RUN_LEVEL != 3 & file.exists("model1/run_level_3/lee1_end_fit.rds")) {
  ...
  lee1_end_mif <- bake(
    ...
    {
      foreach(  # Parallel execution of the IF2 algorith. 
        i = 1:nrow(end_start),
        .packages = c('pomp'),
        .combine = rbind
      ) %dopar% {

        # Get the ith set of starting parameters for the endemic model. These 
        # correspond to the parameters resulting from the ith execution of the 
        # IF2 algorithm on the epidemic model
        r_params <- end_start[i, ]

        # Calculate the starting states for each set of IF2 parameters. Because 
        # there are <N_LEE_SIMS> simulations for each set of parameters, we 
        # take the median value for each set of parameters as the starting 
        # value for the endemic model. 
        lee1_epi_mif$start_states %>%
          filter(param_set == i) %>%
          select(-param_set) %>%
          sapply(., median, na.rm = TRUE) -> rinit_parms

        # Set the initial state by creating a new rinit function for endemic 
        # model. 
        new_rinit <- Csnippet(
          sprintf(
            "
        ...
        # Create a new pomp object representing the endemic model, where the 
        # starting state is the median of the latent states from the epidemic 
        # model for the parameter set i. 
        h1_end <- h1_end %>% pomp(
          rinit = new_rinit,
          statenames = c(
            "S", "E", "I", "A", "R", "incid",
        ...
        # Set model coefficients to be the parameter set i.
        ...
        mf_end <- mif2(
          ...
          cooling.fraction.50 = 0.05,  # Matches Lee et al (2020)
          rw.sd = end_rw
        )
...
The groups largely adhered to existing guidelines on creating models to inform policy \citep{behrend20,saltelli20} and, despite their autonomy, obtained a consensus that an extensive nationwide vaccination campaign would be necessary to eliminate cholera from Haiti.
This conclusion is inconsistent with the fact that there have been no confirmed cases since February, 2019 \citep{trevisin22} without the implementation of a national vaccination program.
...
# Check first if RUN_LEVEL_3 file exists in directory, if not, create the
# RUN_LEVEL version of the results and save them.
...
Calibrating model parameters does not guarantee that the resulting model successfully approximates real-world mechanisms, since model assumptions may be incorrect, and do not change as the model is calibrated to data.
However, the congruency between the model and observed data serves as a proxy for the congruency between the model and the true underlying dynamic system, since the observed data are a result of the system.
...
To our knowledge, the only plug-and-play frequentist methods that can maximize the likelihood for POMP models of this complexity are iterated filtering algorithms, which modify the well-known particle filter \citep{arulampalam02} by performing a random walk for each parameter and particle.
...
However, a legitimate preference for simplicity \citep{saltelli20,green15} can support a decision to consider nationally aggregated models.
...
To avoid the parameter estimation issue in high-dimensional models, \citet{lee20} simplified the problem by using the IF2 algorithm to estimate parameters separately for independent department-level models;
...
This approach of model calibration and forecasting avoids the issue of particle depletion by simplifying the parameter estimation problem, but this advantage raises new issues.
...
For our analysis, we calibrate the parameters of the spatially coupled version of Model~3 using the iterated block particle filter (IBPF) algorithm of \cite{ionides22}.
...
# Check if run_level_3 results exit, and use them if they do.
  ...
  h3Spat_quants <- bake(
    file = paste0('model3/', rl_dir, 'SpatPompSims.rds'), {
      ...
      h3Spat_sims %>%
        ...
        # filter(
        #   date >= as.Date(lubridate::round_date(lubridate::date_decimal(h3Spat@t0), unit = 'day'))
        #   ) %>%
...
# TODO: Not needed after fitting Model 3.
...
# TODO: The code below will not be needed once we update the fitting of model 3.
...
Forecasts aim to provide an accurate estimate of the future state of a system based on currently available data, together with an assessment of uncertainty.
Mechanistic forecasting models, compatible with current scientific understanding, may also provide estimates of the future effects of potential interventions.
...
Epidemiological forecasts based on deterministic models are not uncommon in practice, despite their limitations \citep{king15}.
That may explain why \citet{lee20} chose to obtain forecasts by simulating the calibrated models forward from initial conditions.
Here, we compare with a forecast projected from the filtered distribution: to obtain a forecast using POMP model $m$ at a collection of times $t_{N+1:N+s}$, where $N$ is the index for the last available data, we simulate forward from a draw from
$f^{(m)}_{\bm{X}_N|\bm{Y}_{1:N}}(\bm{x}_{N} | \bm{y}^*_{1:N} ; \hat\theta)$
where $\hat\theta$ is a vector of calibrated parameters.
The decision not to do this partially explains the unsuccessful forecasts of \cite{lee20}: their Table~S7 shows that the subset of their simulations which were consistent with observing zero cases in 2019 predicted the elimination of cholera.
In this case study, projecting the future state of the cholera epidemic starting from latent states that are draws from the filtering distribution allows the model-based forecasts to benefit from the fact that very few cholera cases were observed in 2018 and January 2019.
...
For frequentist inference, an empirical Bayes approach leads to similar methodology \citep{king15}; details are provided in the supplement.
Both of these approaches share the similarity that parameters are chosen for the forecast approximately in proportion to their corresponding value of the likelihood function, $f^{(m)}_{\bm{Y}_{1:N}}(\bm{y}_{1:N}^*; \theta)$.
...
df_rain <- haitiRainfall %>%
  # dplyr::summarize(
  #   # Standardize rainfall value using the function above
...
The depletion of susceptible individuals may also be attributed to confounding mechanisms---such as localized vaccination programs and non-pharmaceutical interventions that reduce cholera transmission \citep{trevisin22, rebaudet21}---that were not accounted for in the model.
...
The ongoing global COVID-19 pandemic has provided a clear example on how government policy may be affected by the conclusions of scientific models \citep{saltelli20}.
...
\begin{supplement}
\stitle{Auxiliary software: \code{haitipkg}}
\sdescription{
The \code{haitipkg} R package is maintained in a GitHub repository, \code{jeswheel/haitipkg}. The submitted version of this package will be archived on Zenodo.
The package contains all of the data and code used to create and fit the models, as well as other useful functions that were used in this article.
}
\end{supplement}

\begin{supplement}
\stitle{Dynamic document: \code{haiti\_article}}
\sdescription{
The Rnw \citep[R noweb,][]{ramsey94} files generating this article and its supplement are maintained in a GitHub repository, \code{jeswheel/haiti\_article}. The submitted version will be archived on Zenodo.
}
\end{supplement}
