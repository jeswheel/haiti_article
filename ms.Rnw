...
Thus, we encounter the methodological task of building credible, data-informed decisions for stochastic, partially observed, nonlinear dynamic models.
...
% \eic{EDIT IN THE DIRECTION OF THE PREVIOUS SENTENCE.}
...
% with the help of vaccination and other public health measures.
% \jwc{Kind of the point here is that it went away without vaccination efforts.}
...
potential effect of increased oral cholera vaccine (OCV) efforts across Haiti.
\jwc{New sentence:}
OCV is accepted as a safe and effective tool for controlling the spread of cholera, however the available stockpile and production of OCV doses is insufficient to meet global needs \citep{lee20, pezzoli20}.
...
We demonstrate that careful attention in model fitting, diagnosing, and forecasting improves each of the proposed model's ability to quantitatively describe observed data and results in forecasts that are more consistent with the observed outcome.
\jwc{NEXT TWO SENTENCES ARE NEW: }
Because we focus on the statistical tasks of model fitting, diagnosing, and forecasting, we refrain from commenting on the feasibility of scientific assumptions implicitly made in the models under consideration.
Our results demonstrate that model-mispecification does not alone explain the discrepancy between model predictions and reality, as has been suggested \citep{rebaudetComment20}.
...
The latent state vector $\bm{X}^{(\modelCounter)}$ consists of individuals labeled as susceptible (S), infected (I), asymptomatically infected (A), vaccinated (V), and recovered (R), with various sub-divisions sometimes considered in each model for $\modelCounter \in \{1, 2, 3\}$.
Models~2 and~3 have metapopulation structure, meaning that each individual is a member of a spatial unit, denoted by $u\in \seq{1}{U}$, which corresponds to membership of the $U=10$ administrative Haitian departments (anglicized version of departements).
% Model~1 describes the dynamics aggregated at the national level.

% \jwc{Already stated above}
...
\jwc{NEW PARAGRAPH:}
In the following subsections, detailed descriptions of Models~1--3 are provided.
While these descriptions are a necessary inclusion for transparency and reproducibility, they do not immediately contribute to our primary goal of model fitting, diagnosing, and forecasting, with the purpose of informing policy.
A first-time reader may therefore benefit from quickly skimming through Sec.~\ref{sec:methods} and returning to the section later, as needed.
...
In \eqref{eq:model3:water}, $J_u(t)$ is a unit-less measurement of precipitation that has been standardized by dividing the observed rainfall at time $t$ by the maximum recorded rainfall in department $u$ during the epidemic, and $D_u$ is the average population density.
...
\jwc{NEW PARAGRAPH}
% Mechanistic modeling is an attempt to account for known or hypothesized features of a dynamic system.
Proposed mechanistic structures form a family of statistical models that can be indexed by a parameter vector $\paramVec$.
Because different values of $\paramVec$ result in drastically different modeling outcomes, researchers must be careful when choosing $\paramVec$.
Elements of $\paramVec$ are either chosen as constants that reflect our understanding of the system, or calibrated to observed data, with the intent of maximizing a proposed model's ability to quantitatively describe observed data according to some measure.
While this section is focused on the elements of $\paramVec$ that are calibrated to data, we note that the decision of which elements to fix and which to estimate has consequences for model interpretability, as briefly discussed in Sec.~\ref{sec:results}

The complex nature of the dynamic systems which are of scientific and public interest necessitates the imposition of various modeling assumptions in order to facilitate inference.
...
For example, one common assumption in epidemiology is a homogeneous mixing population \citep{bansal07}.
...
Ultimately, these decisions may result in a different approach to calibrating model parameters.

\jwc{MAJOR CHANGE TO THIS PARAGRAPH:}
Each model considered in this study describes cholera dynamics as a Partially Observed Markov Process (POMP), as each model contains unobservable states that evolve sequentially with time.
...
% rather than modeling the mean behavior of the system through the use of deterministic differential equations,
...
Model~2 is a deterministic model, which can be viewed as a special case of a POMP (or SpatPOMP) with no randomness in the dynamic model.
...
\jwc{NEW PARAGRAPH/EDIT: }
The purpose of parameter calibration, irrespective of the approach taken, frequentist versus Bayesian), is to ensure that the proposed model provides the best possible description of the observed data under the assumptions of the model, as measured by some metric.
Obtaining the best set of parameters for a given model does not, however, guarantee that the model provides an approximate representation of the system in question.
Model mispecification, which may be thought of as the omission (or inclusion) of a mechanism in the model that is an important (or distracting) feature of the dynamic system, is inevitable at all levels of model complexity.
In fact, we echo the sentiment of George Box, to whom the aphorism ``all models are wrong, but some are useful" is often attributed: we assume that some degree of model mispecification is always present, and it is necessary to subjectively determine whether or not the model mispecification is severe enough to render the model ``useless".
It is often impossible to know, however, the extent to which a model provides a faithful representation of the system in question.
...
While these comparisons are informative, they provide only a naive measure of the goodness-of-fit of a model.
A relevant example of these comparisons leading to overconfidence in a model's goodness-of-fit is the original parameter estimates provided by \citet{lee20}:
these parameters resulted in simulations that closely resembled the observed data, yet resulted in model likelihoods that were---in some cases---remarkably smaller than likelihoods that can be achieved via the likelihood based optimization techniques that were used (see Table~\ref{tab:likes}).
...
Another approach is to compare a quantitative description of the model fit (such as MSE, predictive accuracy, or model likelihood) to that of other proposed models.
...
In the case of mechanistic modeling, however, comparing models based on an objective measure is less natural because each model is designed to provide a scientific description of the dynamic system.
...
The comparison across models still only provides a relative measure of model fit compared to other mechanistic models.
...
Because of this, we propose the use of a log-linear ARMA model as a potential benchmark.
Despite being an associative model, the reason that an ARMA model is an appealing benchmark is because the performance of this class of models is widely understood, and often provides a good description of the data with very limited effort.
...
mod2_epi_ll <- -epi_ofun(par = h2_fit$epi_params)
mod2_end_ll <- -end_ofun(par = h2_fit$end_params)


mod2_ll <- mod2_epi_ll - sum(apply(log(h2_epi@data + 1), 1, sum)) + (mod2_end_ll - sum(apply(log(h2_end@data + 1), 1, sum, na.rm = TRUE)))
...
\jwc{NEW PARAGRAPH:}
Forecasting is a powerful tool that has primarily been used to estimate the future state of a system; when a mechanistic model is used, forecasts may provide estimates of the potential effects interventions may have on a system.
Forecasts can also be used as a way of testing scientific hypothesis in real time by comparing simulations from models with competing descriptions of a system to newly obtained data \citep{lewis22}.
In order to have any benefit to the researcher, however, forecasts must be obtained with great care.
In particular, researchers should be careful to account for various forms of uncertainty present in model forecasts, carefully calibrate the proposed model to observed data, and---to the best of their ability---account for the most recently known state of the system.
...
These results suggest that cholera elimination was likely, even without increased vaccination efforts, which is consistent with observed reality \citep{ferguson22}.
...
Seasonality
  ...
  & \eqref{model2:lambda}
  & $\begin{array}{l} \seasAmplitude=\Sexpr{myround(p3s["lambdaR"],2)} \\ r=\Sexpr{signif(p3s["r"],2)} \end{array}$
...
\end{array}$
  ...
  & {\fixed
    $\begin{array}{l}
    \muRS^{-1}= \Sexpr{signif(1/h2_fit$epi_params["sigma"],2)} \\
...
\end{array}$
  ...
  & $\begin{array}{l}
      ...
      \Sexpr{signif(52/h2_fit$end_params["Delta"],1)}
      )
      \end{array}$
  & \eqref{model2:from_W}
