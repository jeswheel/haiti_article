...
Each model describes the cholera dynamics using a latent state vector $\bm{X}^{(\modelCounter)}(t)$ for each continuous time-point $t$ and $\modelCounter \in \{1, 2, 3\}$ indexes the model.
The $N$ observation times are denoted by $t_1,\dots,t_N$ which is also written as $t_{1:N}$.
The observation at time $t_n$ is modeled by a response vector $\bm{Y}^{(\modelCounter)}_n$.
The latent state vector $\bm{X}^{(\modelCounter)}(t)$ consists of individuals labeled as susceptible (S), infected (I), asymptomatically infected (A), vaccinated (V), and recovered (R), with various sub-divisions sometimes considered in each model $\modelCounter$.
...
We consider model fitting (Sec.~\ref{sec:model_fitting}) followed by diagnostic investigations (Sec.~\ref{sec:model_diagnostics}), forecasting (Sec.~\ref{sec:filter}) and \eic{PERHAPS external scientific corroboration of model fit RATHER THAN interpreting model fit} (Sec.~\ref{sec:science}).
...
if (RUN_LEVEL == 1){
  ...
  # Local search, all parameters
  SEARCH2 <- list(
    TOP_N = 12, NP = 2000, NMIF = 100, NREPS = 6, NREPS_EVAL = 36, NP_EVAL = 3000,
    RW_SD = rw.sd(
      betaB = 0.0075,   # Unit Specific
      foi_add = 0.0075, # Unit Specific
      mu_B = 0.0075,
      thetaI = 0.0075,
      XthetaA = 0.0075,
      lambdaR = 0.0075,
  ...
  # Local search, unit specific parameters
  SEARCH3 <- list(
    TOP_N = 9, NP = 2000, NMIF = 100, NREPS = 8, NREPS_EVAL = 36, NP_EVAL = 4000,
    RW_SD = rw.sd(
      betaB = 0.005,
      foi_add = 0.005
    )
...
\eic{PERHAPS THIS PARAGRAPH COULD MOVE TO A LATER SECTION? THE FOLLOWING PARAGRAPH IS MORE WHAT I EXPECTED READING THROUGH WHEN I REACHED THIS POINT IN THE MS. PERHAPS PARTS OF THIS FIRST PARAGRAPH ARE EVEN REDUNDANT, IF THEY ARE COVERED IN THE ORIGINAL DISCUSSION OF THE MODELS.}
...
Each of the three models considered in this study describes cholera dynamics via unobservable states that evolve dynamically with time.
...
\eic{ALL THE MODELS ARE PROBABILISTIC IN SOME SENSE, EVEN THOSE WITH DETERMINISTIC DYNAMICS. HAVING POINTED OUT THAT THEY ARE ALL POMP MODELS ABOVE, MAYBE WE SHOULD DEFINE THE CORRESPONDING NOTATION ABOVE TOO, RATHER THAN SPECIFICALLY FOR MODEL 1}
One approach to modeling a dynamic system is through probabilistic models.
With such a model, the existence of a joint density $f_{\bm{X}^{(\modelCounter)}_{\seq{0}{N}}, \bm{Y}^{(\modelCounter)}_{\seq{1}{N}}}$ is supposed, with $\bm{X}^{(\modelCounter)}_{\seq{0}{N}}$ denoting the unobservable Markov process of model $\modelCounter$ at times $\seq{0}{N} = \{0, 1, \ldots, N\}$, and $\bm{Y}^{(\modelCounter)}_{\seq{1}{N}}$ denoting the observable process of the system at times $\seq{1}{N}$.
Under this framework, the observed data $y_{\seq{1}{N}}^*$---along with some unobserved value $\bm{X}^{(\modelCounter)}_{\seq{0}{N}} = x^*_{\seq{0}{N}}$---are assumed to be a single realization of the model $\big(x^*_{\seq{0}{N}}, y_{\seq{1}{N}}^*\big) \sim f_{\bm{X}^{(\modelCounter)}_{\seq{0}{N}}, \bm{Y}^{(\modelCounter)}_{\seq{1}{N}}}(x_{\seq{0}{N}}, y_{\seq{1}{N}}; \paramVec)$, where $\paramVec$ is a parameter vector that indexes the model.
This modeling framework results in several advantages, including the ability to account for variability present in the system.
...
\eic{MAYBE THE KEY THING HERE IS THAT MODEL 1 IS A ``STANDARD'' POMP MODEL, FOR WHICH MUCH SUCCESSFUL METHODOLOGY IS WIDELY AVAILABLE. THAT IS BASICALLY WHAT WE PROCEED TO SAY.}
...
The ability to maximize the likelihood allows for likelihood-based inference, like \eic{I THINK like IS TOO INFORMAL AND PERHAPS ALSO VAGUE; e.g. OR for example WOULD NOT WORK HERE; MAYBE including?} performing statistical tests for potential improvements to the model.
...
\eic{BEST TO AVOID NOTATION MORE MATHEMATICALLY ABSTRACT THAN NECESSARY - WE WANT THIS TO BE ACCESSIBLE TO PEOPLE WITHOUT MUCH ANALYSIS EXPERIENCE. ALSO, THIS IS NOT QUITE CORRECT; PERHAPS WE MEAN $[t_0,t_N]$}
...
Aside from the introduction of the parameter $\transmissionTrend$ and the multiplicative Gamma-distributed white noise $d\Gamma(t)/dt$ in the process model (Eq.~\ref{model1:lambda}), the another change we made to increase the likelihood of the fitted model was using a substantially increased computational effort in the numerical maximization.
\eic{LET'S DISCUSS HOW THIS SENTENCE CAN BE MISREAD AT FIRST PASS (AND ARGUABLY IS AMBIGUOUS EVEN AFTER MORE THOUGHT). I THINK THIS IS COMMONLY THE CASE IN SENTENCES WITH MULITPLE SUBORDINATE CLAUSES} While flexible, plug-and-play methods like IF2 require a large number of computations, and the theoretic ability to maximize the likelihood depends on asymptotics in both the number of particles used and the number of filtering iterations.
...
Model~2 is a deterministic model, which are degenerate probabilistic models that contain no randomness in the dynamic process or measurement \eic{HERE, WE USE A MEMBER OF A CLASS AS A SHORT-CUT FOR THE CLASS ITSELF, WHICH READS STRANGELY AND FORMALLY HAS A PLURALITY MIS-MATCH.}.
...
This issue is particularly troublesome for this case study: the inability to accurately fit times of low infectiousness may result in poor model forecasts, as few cases of cholera were observed the last few years of the epidemic.

To combat this issue, we fit the model to log-transformed case counts, where the difference between periods of high and low infectiousness is less drastic \eic{, since the log scale stabilizes the variation during periods of high and low infectiousness?? I'M NOT ENTIRELY HAPPY WITH THAT EITHER}.
...
To avoid the parameter estimation issue in high-dimensional models, \citet{lee20} simplified the problem by using the MIF2 \eic{IF2} algorithm to separately estimate parameters for independent department-level models;
...
A first order \eic{IS THIS BEING USED IN A TECHNICAL SENSE HERE?} approximation of Eq.~\myeqref{eq:model3:foi} for each department $v \in \seq{1}{10}$ can be obtained using the weekly number of observed cholera cases in each department:
\begin{equation}
...
$P\big(\Theta_{j} = \theta_k | \bm{Y}^{(\modelCounter)}_{1:N} = y_{1:N}^*\big) = \frac{l(y_{1:N}^*; \theta_k)}{\sum_{l = 1}^K l(y_{1:N}^*; \theta_l)}$. To complete the formulation, we then choose the set $\Theta_K$ such that each $\theta_k \in \Theta_K$ is a single output of the MIF2 algorithm, which has been shown to be within a neighborhood of the MLE \cite{ionides15}.
