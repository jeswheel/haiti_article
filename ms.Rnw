...
# TODO: update this comment: This is used later in figure 2 (TODO)
...
# Load the file containing profiles of Model 1 Parameters.
if (RUN_LEVEL != 3 && file.exists("model3/run_level_3/h3_MLE_eval.rds")) {

  # If RUN_LEVEL != 3, and RUN_LEVEL == 3 results exist, just load the RL3
  # results.
  h3_prof <- readRDS("model3/run_level_3/h3_MLE_eval.rds")
} else if (file.exists(paste0('model3/', rl_dir, 'h3_MLE_eval.rds'))) {

  # If the `model3/run_level_<RUN_LEVEL>/h3_profiles.rds` file exists, load it
  h3_prof <- readRDS(paste0('model3/', rl_dir, 'h3_MLE_eval.rds'))
} else {

  # Otherwise (missing RL3 file, and the corresponding <RUN_LEVEL> file):
  # Throw an error, letting users know how to create the file.
  stop(paste0("Missing file: `model3/", rl_dir, 'h3_MLE_eval.rds`;\nThe file can be obtained by running the script `H3_MLE_Eval.R`.'))
}
