...
# run_level_* for model 2 is not needed.
for (i in c(1, 3)) {
...
if (RUN_LEVEL != 3 & file.exists("model1/run_level_3/lee1_epi_fit.rds")) {
  ...
  lee1_epi_mif <- bake(
    file = paste0("model1/", rl_dir, "lee1_epi_fit.rds"),
    {
      ...
      foreach(i = 1:length(lee1_epi_mif_out), .combine = rbind) %do% {
        guess <- coef(lee1_epi_mif_out[[i]])
        sims <- simulate(
          h1_epi,
          n = N_LEE_SIMS,
          format = 'data.frame',
      ...
      lee1_epi_mif_results$lee1_epi_mif_params <- t(coef(lee1_epi_mif_out))
      lee1_epi_mif_results$start_states <- start_states
      lee1_epi_mif_results
    }
  )
}
...
if (RUN_LEVEL != 3 & file.exists("model1/run_level_3/lee1_epi_evals.rds")) {
  ...
  lee_epi_ll <- bake(
    file = paste0("model1/", rl_dir, "lee1_epi_evals.rds"), {
      ...
      for (j in 1:nrow(lee1_epi_mif$lee1_epi_mif_params)) {
        # Get parameters of interest
        pf_params <- lee1_epi_mif$lee1_epi_mif_params[j, ]
...
end_start <- lee1_epi_mif$lee1_epi_mif_params
  ...
  lee_end_ll <- bake(
    file = paste0("model1/", rl_dir, "lee1_end_evals.rds"), {
      ...
      lee_end_ll <- data.frame(
        "pfLL" = rep(0, nrow(lee1_end_mif)),
        "pfse" = rep(0, nrow(lee1_end_mif))
      )

      # Perform particle filter to get likelihood estimates
      for (j in 1:nrow(lee_end_ll)) {
        # Get parameters of interest
        pf_params <- lee1_end_mif[j, ]
...
rm(
  lee_end_ll, lee_epi_ll, h1_end, h1_epi, end_start,
...
This conclusion is inconsistent with the fact that there have been no confirmed cases since February, 2019 \citep{trevisin22}, despite the lack of a national vaccination program.

The discrepancy between the model based conclusions of \cite{lee20} and the subsequent elimination of cholera cholera has been debated \citep{francois20,rebaudetComment20,henrys20,leeReply20}.
Suggested origins of this discrepancy include the use of unrealistic models \citep{rebaudetComment20} and having unrealistic criteria for cholera elimination \citep{henrys20}.
...
These improvements result in forecasts that are more consistent with the observed outcome, without requiring major changes to the model structures.
...
While this work is focused on the models proposed by \cite{lee20}, our suggestions have broader relevance.
To investigate the extent to which \citet{lee20} is typical of the substantial body of work on Haiti cholera dynamics, we preformed a literature review by searching PubMed with keywords: Haiti, cholera, model.
The search resulted in 66 papers, of which 32 used dynamic models to describe the cholera epidemic in Haiti.
The models make various choices on the dichotomies considered by \citet{lee20}; deterministic versus stochastic; compartment model versus agent-based; aggregated versus spatially explicit.
This is no accident, since \citet{lee20} purposefully designed their study to cover the range of current modeling practice.

We proceed by introducing Models~1--3 in Sec.~\ref{sec:models};
in Sec.~\ref{sec:methods}, we present a systematic approach to examining and refining these models, and then use improved model fits to project cholera incidence in Haiti under various vaccination scenarios.
...
This motivates the development of mechanistic statistical models, which provides a statistical fit to the available data while also supporting a causal interpretation.
...
Each model describes the cholera dynamics as a partially observed Markov process (POMP) with a latent state vector $\bm{X}^{(\modelCounter)}(t)$ for each continuous time point $t$ and model $\modelCounter \in \{1, 2, 3\}$.
$N$ observations on the system are collected at time points $t_1,\dots,t_N$, written as $t_{1:N}$.
The observation at time $t_n$ is modeled by the random vector $\bm{Y}^{(\modelCounter)}_n$.
...
We therefore write $\bm{X}^{(\modelCounter)}_n = \bm{X}^{(\modelCounter)}(t_n)$ to denote the value of the latent process at the $n$th observation time, and $\bm{X}^{(\modelCounter)}_{1:N}$ is the collection of latent state values for all observed time points.
The observable random variables $\bm{Y}^{(\modelCounter)}_{1:N}$ are assumed to be conditionally independent given $\bm{X}^{(\modelCounter)}_{0:N}$.
Together, with the density for the initial value of the latent state $\bm{X}^{(\modelCounter)}_{0} = \bm{X}^{(\modelCounter)}(t_0)$, each model assumes a joint density $f_{\bm{X}^{(\modelCounter)}_{0:N}, \bm{Y}^{(\modelCounter)}_{1:N}}\big(\bm{x}^{(\modelCounter)}_{0:N}, \bm{y}^{(\modelCounter)}_{1:N};\paramVec\big)$, where $\paramVec$ is a parameter vector that indexes the model.
...
For each model $\modelCounter \in \{1, 2, 3\}$, the latent state vector $\bm{X}^{(\modelCounter)}(t)$ consists of individuals labeled as susceptible (S), infected (I), asymptomatically infected (A), vaccinated (V), and recovered (R), with various sub-divisions sometimes considered.
...
In the following subsections, descriptions of Models~1--3 are provided.
While the model description is scientifically critical, as well as necessary for transparency and reproducibility, the model details are not essential to our methodological discussions of how to diagnose and address model misspecification with the purpose of informing policy.
...
where $\theta^*(t)$ is efficacy at time $t$ since vaccination for adults, taken from \cite{lee20}, Table~S4, and $c=\big(1-(1-0.4688)\times 0.11\big)$ is a correction to allow for reduced efficacy in the 11\% of the population aged under 5 years.
...
The latent state vector $\bm{X}^{(1)}(t)$ is initialized by setting each compartment count for each vaccination scenario $\vaccCounter \neq 0$ as zero, and introducing initial-value parameters $I_{0,0}$ and $E_{0, 0}$ such that $R_{0}(0) = 0$, $I_{0}(0) = \mathrm{Pop} \times I_{0, 0}$, $E_{0}(0) = \mathrm{Pop} \times E_{0, 0}$ and $S_{0}(0) = \mathrm{Pop} \times (1 - I_{0, 0} - E_{0, 0})$, where $\mathrm{Pop}$ is the total population of Haiti.
Reported cholera cases at time point $n$ ($\bm{Y}^{(1)}_n$) are assumed to come from a negative binomial measurement model, where only a fraction ($\reportRate$) of new weekly cases are reported.
See Sec.~S4 of the supplement material for more details.
...
The \cite{lee20} implementation of Model~2 fixes $\phaseParm = 0$.
...
$D_{u\varv}$ is a distance measure estimating average road distance between randomly chosen members of each population, and $v_{\mathrm{rate}}= 10^{-12}$ was treated as a fixed constant.
...
The starting value for each element of the latent state vector $\bm{X}^{(2)}(0)$ are set to zero except for $I_{u0}(0) = y_u^*(0)/\reportRate$ and $R_{u0}(0) = \mathrm{Pop}_u - I_{u0}(0)$, where $y_u^*(0)$ is the reported number of cholera cases in department $u$ at time $t = 0$.
...
See Sec.~\ref{sec:fit2} and Sec.~S4.2 of the supplement for more details.
...
\end{equation}
and Per-capita transition rates are given in Equations~\ref{eq:model3:SI}--\ref{eq:model3:Decay}.
\begin{eqnarray}
\label{eq:model3:SI}
...
\mu_{{\demography}W_u} &=& \big[1 + \seasAmplitude \big(J_u(t))^r \big] \mathrm{Den}_u \, \Wshed \big[ I_{u0}(t)+ \asymptomRelativeShed A_{u0}(t) \big],
\\[-3pt]
...
In \eqref{eq:model3:water}, $J_u(t)$ is a dimensionless measurement of precipitation that has been standardized by dividing the observed rainfall at time $t$ by the maximum recorded rainfall in department $u$ during the epidemic, and $\mathrm{Den}_u$ is the population density.
...
Latent states are initialized using an approximation of the instantaneous number of infected, asymptomatic, and recovered individuals at time $t=0$ by using the first week of cholera incidence data.
Specifically, we set $I_{u0}(0) = \frac{y^*_{u0}}{\reportRate(\muDeath + \choleraDeath + \muIR)}$, $A_{u0}(0) = \frac{1 - \symptomFrac}{\symptomFrac}I_{u0}(0)$, and $R_{u0k} = y^*_{u0} - I_{u0}(0) - A_{u0}(0)$, and all other compartments that represent population counts are set to zero at time $t=0$.
We replace $J_u(0)$ in \eqref{eq:model3:water} with initial value parameters $\Binit_u$ to allow for some flexibility in initializing the bacterial compartment $W_{u}(0)$ while still maintaining the primary dynamics of the model.
Reported cholera cases are assumed to come from a negative binomial measurement model with mean equal to a fraction ($\reportRate$) of individuals in each unit who develop symptoms seek healthcare (see Sec.~S4 of the supplement material for more details).

\section{Statistical Analysis}\label{sec:methods}

We consider model fitting (Sec.~\ref{sec:model_fitting}) followed by diagnostic investigations (Sec.~\ref{sec:model_diagnostics}), forecasting (Sec.~\ref{sec:filter}) and external scientific corroboration of model fit (Sec.~\ref{sec:science}).
...
setrw1 <- function(x) {
  if (grepl('^r[[:digit:]]+$', x)) {
    return(0.01 / 2)
  } else if (grepl('^Binit[[:digit:]]+$', x)) {
    return(expression(ivp(0.02)))
  } else {
    return(0.01)
  }
}
...
if (RUN_LEVEL != 3 & file.exists("model3/run_level_3/haiti3_coupled_fit.rds")) {
  haiti3_fit <- readRDS("model3/run_level_3/haiti3_coupled_fit.rds")
} else {
  haiti3_fit <- bake(
    file = paste0("model3/", rl_dir, "haiti3_coupled_fit.rds"), {
      fit_coupled_haiti3(
        nsearches = N_SEARCHES,
        search1 = SEARCH1,
        search2 = SEARCH2,
        search3 = SEARCH3,
        ncores = cores
...
rm(
  SEARCH1, SEARCH2, SEARCH3, N_SEARCHES, unit_specific_names, 
  shared_param_names, est_param_names, est_param_names_expanded, 
  setrw1, setrw2, setrw3, reg_rw_1.sd, reg_rw_2.sd, reg_rw_3.sd, 
...
p3_specific <- c("foi_add", "betaB", "Binit")
p3_shared <- c(
  "mu_B", "XthetaA", "thetaI", "lambdaR", "r", "std_W",
  "epsilon", "k", "sigma"
)

p3u <- matrix(nrow = length(p3_specific), ncol = 10)
rownames(p3u) <- p3_specific
p3s <- numeric(length(p3_shared))
names(p3s) <- p3_shared

for (p in p3_shared) {
  pat <- paste0("^", p, "[[:digit:]]+$")
  p3s[p] <-  mean(p3[grepl(pat, names(p3))])
}

for (p in p3_specific) {
  pat <- paste0("^", p, "[[:digit:]]+$")
  p3u[p, ] <- p3[grepl(pat, names(p3))]
}

rm(p3_specific, p3_shared, p, pat)
...
% \eic{PERHAPS THIS PARAGRAPH COULD MOVE TO A LATER SECTION?}
...
% For example, many models commonly used in epidemiology are motivated by reasoning about a homogeneous mixing population which is simultaneously an avenue for powerful simplification and a source of model misspecification \citep{bansal07}.
...
Each of the three models considered in this study describes cholera dynamics as partially observed Markov processes (POMP), with the understanding that the deterministic Model~2 is a degenerate case of a stochastic model.
...
As such, each proposed mechanistic structure forms a family of statistical models indexed by a parameter vector $\paramVec$.
...
Specific elements of $\paramVec$ can be chosen as constants based on scientific understanding of the system, but parameters can also be calibrated to data in a way that maximizes some measure of congruency between the observed data and the assumed mechanistic structure.
Mechanistic models are used as a way of approximating real-world mechanisms present in a dynamic system.
While calibrating model parameters does not directly achieve this goal---as model assumptions may be incorrect, and do not change as the model is calibrated to data---the congruency between the model and observed data works as a proxy for the congruency between the model and the true underlying dynamic system, since the observed data are a result of the system.
...
In the following subsections we describe our approach to fitting the three proposed mechanistic models by calibrating $\paramVec$ to observed cholera incidence data.

\subsubsection{Calibrating Model~1 Parameters}\label{sec:fit1}

Model~1 describes cholera dynamics at the nationally aggregated scale so that $\bm{Y}^{(1)}_n \in \R$ for each observation time $n \in 1:N$, and the latent state vector $\bm{X}^{(1)}(t)$ is comprised of national level population counts.
Several algorithms exist, both frequentist and Bayesian, that can be used to obtain estimates of the parameters for the class of models.
...
\cite{lee20} implemented Model~1 by fitting two distinct phases: an epidemic phase from October 2010 through March 2015, and an endemic phase from March 2015 onward.
...
We implemented Model~1 using the \code{pomp} package \cite{pomp2}, relying heavily on the \code{pomp} source code provided by \cite{lee20}.
...
While IF2 is a powerful method that can be used to fit parameters for a large class of models, the theoretic ability to maximize the likelihood depends on asymptotics in both the number of particles and the number of filtering iterations and therefore requires a large number of computations.
The large difference in likelihoods that were obtain by these two approaches (see Table~\ref{tab:likes}) highlights the importance of carefully determining the necessary computational effort needed to maximize model likelihoods and acting accordingly.
...
As previously stated, Model~2 is a deterministic compartmental model defined by a set of coupled differential equations.
The use of deterministic compartment models have a long history in the field of infectious disease epidemiology \citep{kermack1927,brauer2017,varghese21},
and can be justified by asymptotic considerations in a large-population limit \citep{dadlani2020,ndii17}.
Because the process model of Model~2 is deterministic, the parameter estimation problem for Model~2 reduces to a least squares calculation when combined with a Gaussian measurement model (see supplement material for details).
...
The inclusion of a model break-point by \cite{lee20} is perhaps due to a challenging feature of fitting a deterministic model via least squares: discrepancies between model trajectories and observed case counts in highly infectious periods of a disease outbreak will result in greater penalty than the discrepancies between model trajectories and observed case counts in times of relatively low infectiousness, resulting in a bias towards accurately describing periods of high infectiousness.
This issue is particularly troublesome for modeling cholera dynamics in Haiti: the inability to accurately fit times of low infectiousness may result in poor model forecasts, as few cases of cholera were observed the last few years of the epidemic.

To combat this issue, we fit the model to log-transformed case counts, where the difference between periods of high and low infectiousness is less drastic, since the log scale stabilizes the variation during periods of high and low infectiousness.
...
Here we chose to fit the model to transformed case counts rather than adding overdispersion to the measurement model with the goal of making the fewest changes to the model proposed by \cite{lee20} as possible.
...
Both the latent and observable processes in Model~3 can be be factored into department specific processes which interact with each other.
The decision to address metapopulation dynamics via a spatially explicit model, rather than to aggregate over space, is double-edged.
...
A legitimate preference for simplicity \citep{saltelli20,green15} can support a decision to consider nationally aggregated models.
...
Model~3 parameters that are calibrated to data are primarily shared between each department, meaning that the estimated parameter value for department $u$ is the same value as the parameter value in department $v$, with $u, v \in \seq{1}{U}$.
The exception to this being the parameters $\beta_{W_u}$, and $\beta_u$, which take department-specific values for each department $u$.
To avoid the parameter estimation issue in high-dimensional models, \citet{lee20} simplified the problem by using the IF2 algorithm to separately estimate parameters for independent department-level models;
...
To address this issue, \cite{lee20} conditioned each department level model on reported cholera cases in other departments and approximated the values of $I_\nu(t)$ and $A_\nu(t)$ using the respective reported cases.
...
The simplified, spatially-decoupled version of Model~3 relies on the observed cholera cases for each department to describe cholera dynamics, making it impossible to obtain forecasts for future time points from the fitted model.
In order to obtain model forecasts, the parameters fit for the conditionally independent department level models were used in the fully coupled version of Model~3.
This approach of model calibration and forecasting avoids the issue of particle depletion by simplifying the parameter estimation problem, but it may result in some undesirable outcomes.
One concern is that cholera dynamics in department $u$ are highly related to the dynamics in the remaining departments;
calibrating model parameters using observed cases in other departments as a model covariate may therefore lead to an over-dependence on observed cholera cases.
...
This is because the likelihood of the coupled model at its maximum likelihood estimate (MLE) is greater than or equal to the likelihood of the coupled model at the MLE of the decoupled model.
These two concerns may explain the unrealistic forecasts and low-likelihood of the model (see Table~\ref{tab:likes}).

For our analysis, we calibrate the parameters of the spatially coupled version of Model~3 using the novel iterated block particle filter (IBPF) algorithm of \cite{ionides22}.
This algorithm extends the work of \cite{ning21ibpf}, who provided theoretic justification for the version of the algorithm that only estimates unit-specific parameters.
While computationally intensive, the IBPF algorithm enables us to directly estimate the parameters of models describing high-dimensional partially-observed nonlinear dynamic systems via likelihood maximization.
The ability to directly estimate parameters of Model~3 is responsible for the large increase in model likelihoods reported in Table~\ref{tab:likes}, as no algorithm with similar capabilities existed when \cite{lee20} published their results.
Simulations from the fitted model are displayed in Fig.~\ref{fig:h3spatsims}.

% An approximation of Eq.~\myeqref{eq:model3:foi} for each department $v \in \seq{1}{10}$ can be obtained using the weekly number of observed cholera cases in each department:
% \begin{equation}
...
%
% In the case of a PanelPOMP, an extension of the IF2 algorithm, known as Panel Iterated Filtering (PIF) \citep{breto20}, can be used to obtain the MLE, which solves the curse of dimensionality for this class of models \eic{WHICH CONVENTIONALLY REFERS TO THE PRECEDING OBJECT, HERE THE MLE, WHICH IS NOT WHAT WE WANT}.
...
%
% \eic{READERS MAY BE CONFUSED HERE. IS THIS A NEW METHOD? IT SEEMS TO BE IMPLEMENTED BY BRETO ET AL IN 2020. THIS IS DELICATE.}
% We propose and use a slight modification of the PIF algorithm, which we call the Block Panel Iterated Filter (BPIF), to estimate parameters for a PanelPOMP version of Model~3.
...
% The BPIF algorithm is implemented in the \code{panelPomp} package \citep{breto20panelPomp} as the argument \code{block = TRUE} in the \code{mif2} function \eic{THIS LEVEL OF CODING DETAIL IS UNUSUAL IN A CASE STUDY}.
...
% Similar to what was done in \cite{lee20}, we use the parameter estimates obtained with the independent departmental approximation of Model~3 as an estimate for the parameters in the fully coupled version of the model, which was implemented using the \code{spatPomp} package.
...
% The answer to this question requires the ability to reliably estimate $\hat{\paramVec}$, the answer to which is model specific and beyond the scope of this case study.

% A brief analysis of the fitted parameters $\tilde{\paramVec}$ provides some interesting insight.
% Consider the large estimated exponent on rainfall: $r = \Sexpr{signif(p3s["r"],3)}$ (Eq.~\ref{eq:model3:water}).
...
% Whatever the case may be, this fitting procedure results in a model that is unable to replicate the observed number of cases by simulating from initial conditions (see Fig.~\ref{h3spatsims}), and highlights the need for advancements in statistical methodology that permit inference on models with coupled metapopulation dynamics.
...
Beyond being good practical advice for applied statistics, this assertion is relevant for the philosophical justification of statistical inference as severe testing \citep[][Sec.~4.8]{mayo18}.
...
In our literature review of the Haiti cholera epidemic, no benchmark models were considered in any of the 24 papers which calibrated a mechanistic model to data in order to obtain scientific conclusions.
...
The use of benchmarks may also be beneficial when developing models at differing spatial scales, where a direct comparison between models likelihoods is meaningless.
...
Large advantages (shortcomings) in model likelihood relative to the benchmark for a given spatial scale that are not present in other spatial scales may provide weak evidence for (against) the statistical fit of models across a range of spatial resolutions.
...
Additional tools for assessing the goodness-of-fit of a model include plotting the effective sample size of each observation \citep{liu01}, and comparing summary statistics of the observed data to simulations from the model \citep{wood10}.
These summary statistics are sometimes called diagnostic probes \citep{king15,king16}.
...
In order to provide trustworthy information, however, the reliability of the forecast should be understood.
...
Using state space model $m$, this means that in order to obtain a forecast up to time $N+s$, where $s\geq 0$ and $N$ is the index for the last available data, we should simulate from the conditional density $f_{\bm{X}^{(\modelCounter)}_{N:s}, \bm{Y}^{\modelCounter}_{N:s}}\big(x_{N:s}, y_{N:s}\mid \hat{\paramVec}, \bm{X}^{(\modelCounter)}_{N} = \bm{x}^{(\modelCounter)}_N\big)$, where $\bm{x}^{(\modelCounter)}_N$ is the value of the latent state at time $N$.
Because the latent state $\bm{X}^{(\modelCounter)}_N$ is unobservable, draws from the desired conditional density are unobtainable.
...
In this particular case study, projecting the future state of the cholera epidemic starting from latent states that are draws from the filtering distribution allows the proposed models to benefit from the fact that very few cholera cases had been observed between 2018 and 2019, and that cases appear to be decreasing.
...
% It has been noted that the uncertainty in just a single parameter can lead to drastically different forecasts \citep{saltelli20};
% parameter uncertainty should therefore be considered when obtaining model forecasts to influence policy.
% If a Bayesian technique is used for parameter estimation, the most natural way to account for parameter uncertainty is to obtain $J$ simulations from the model where each simulation is obtained using parameters $\Theta_{1:J}$ drawn from the posterior distribution $\Theta_{1:J} \sim f_{\Theta}\big(\paramVec | \bm{Y}^{(\modelCounter)}_{1:N} = y_{1:N}^*\big)$.
% In the frequentist case, one approach is empirical Bayes: endow to set $\Theta_K = \{\theta_1, \theta_2, \ldots, \theta_K\}$ with a discrete uniform distribution, such that $P\big(\Theta_{j} = \theta_k\big) = \frac{1}{K}$ for all values $k \in \seq{1}{K}$.
% In this case, the posterior distribution of $\Theta_{j} | \bm{Y}^{(\modelCounter)}_{1:N}$ can be expressed as:
% $P\big(\Theta_{j} = \theta_k | \bm{Y}^{(\modelCounter)}_{1:N} = y_{1:N}^*\big) = \frac{l(y_{1:N}^*; \theta_k)}{\sum_{l = 1}^K l(y_{1:N}^*; \theta_l)}$. To complete the formulation, we then choose the set $\Theta_K$ such that each $\theta_k \in \Theta_K$ is a single output of the IF2 algorithm, which has been shown to be within a neighborhood of the MLE \cite{ionides15}.
% This approach has been taken in previous studies \citep{king15}, and the result is simply weighing parameter sets based on their corresponding likelihoods: parameters with higher likelihoods should be used more frequently in model forecasts than those that correspond to lower likelihoods.
% This can be done for both deterministic and stochastic models, but requires a large number of computations, especially as the number of observations and model parameters increase.
% Because our direct aim is not to provide forecasts for policy recommendation, we do not include parameter uncertainty in our forecasts.
% Instead, we use the projections from point estimates to highlight a major deficiency of deterministic models, which is that the only variability in model projections is a result of parameter uncertainty, which leads to over-confidence in forecasts \cite{king15}.
...
It has been noted that the uncertainty in just a single parameter can lead to drastically different forecasts \citep{saltelli20};
parameter uncertainty should therefore be considered when obtaining model forecasts to influence policy.
If a Bayesian technique is used for parameter estimation, the most natural way to account for parameter uncertainty is to obtain simulations from the model where each simulation is obtained using parameters drawn from the posterior distribution.
In the frequentist case, one approach is empirical Bayes, as was done by \cite{king15} (see supplement for details). % TODO: \jwc{ADD THIS TO SUPPLEMENT!}
Both of these approaches share the similarity that parameters with higher likelihoods should be used more frequently in model forecasts than those that correspond to lower likelihoods.

Because our direct aim is not to provide forecasts for policy recommendation, we do not include parameter uncertainty in our forecasts.
Instead, we use the projections from point estimates to highlight a major deficiency of deterministic models, which is that the only variability in model projections is a result of parameter uncertainty, which leads to over-confidence in forecasts \cite{king15}.
% To the credit of deterministic models, of the four fitted models in \cite{lee20}, Model~2 provides the most apparently accurate forecasts.
% This perhaps demonstrates that while deterministic models describe the systems in a less realistic way, the relative ease in fitting these models potentially results in fewer modeling errors.
% As the data analysis becomes more refined, however, the deficiencies of deterministic models become increasingly apparent. \jwc{CHANGE THIS BASED ON DISCUSSION WITH ED.}

Forecasting can be used to simulate various interventions on a system; this feature is useful to inform policy and was the primary goal of \citet{lee20}.
...
Mimicking their efforts, we define cholera elimination as having less than one infection of cholera over at least 52 consecutive weeks, and provide forecasts under the following vaccination scenarios:
...
if (RUN_LEVEL != 3 & file.exists("model1/run_level_3/S0_sims.rds")) {
  h1_S0_results <- readRDS("model1/run_level_3/S0_sims.rds")
} else {
  h1_S0_results <- bake(
...
if (RUN_LEVEL != 3 & file.exists("model1/run_level_3/S2_sims.rds")) {
  h1_S2_results <- readRDS("model1/run_level_3/S2_sims.rds")
} else {
  h1_S2_results <- bake(
    file = paste0('model1/', rl_dir, 'S2_sims.rds'), {
      ...
      end_states <- saved.states(s2_pf)
      end_states <- end_states[[length(end_states)]]

      # project vacc scenario, with filtering dist, and model.
      h1_S2_sims <- project_from_filter(mod = s2, end_states = end_states, nsims = h1_scen_NP)
...
if (RUN_LEVEL != 3 & file.exists("model1/run_level_3/S4_sims.rds")) {
  h1_S4_results <- readRDS("model1/run_level_3/S4_sims.rds")
} else {
  h1_S4_results <- bake(
    file = paste0('model1/', rl_dir, 'S4_sims.rds'), {
      ...
      end_states <- saved.states(s4_pf)
      end_states <- end_states[[length(end_states)]]

      # project vacc scenario, with filtering dist, and model.
      h1_S4_sims <- project_from_filter(mod = s4, end_states = end_states, nsims = h1_scen_NP)
...
if (RUN_LEVEL != 3 & file.exists("model1/run_level_3/S3_sims.rds")) {
  h1_S3_results <- readRDS("model1/run_level_3/S3_sims.rds")
} else {
  h1_S3_results <- bake(
    file = paste0('model1/', rl_dir, 'S3_sims.rds'), {
      ...
      end_states <- saved.states(s3_pf)
      end_states <- end_states[[length(end_states)]]

      # project vacc scenario, with filtering dist, and model.
      h1_S3_sims <- project_from_filter(mod = s3, end_states = end_states, nsims = h1_scen_NP)
...
if (RUN_LEVEL != 3 & file.exists("model1/run_level_3/S1_sims.rds")) {
  h1_S1_results <- readRDS("model1/run_level_3/S1_sims.rds")
} else {
  h1_S1_results <- bake(
    file = paste0('model1/', rl_dir, 'S1_sims.rds'), {
      ...
      end_states <- saved.states(s1_pf)
      end_states <- end_states[[length(end_states)]]

      # project vacc scenario, with filtering dist, and model.
      h1_S1_sims <- project_from_filter(mod = s1, end_states = end_states, nsims = h1_scen_NP)
...
if (RUN_LEVEL != 3 & file.exists("model3/run_level_3/h3_bpf_end_states.rds")) {
  h3_bpf_end_states <- readRDS("model3/run_level_3/h3_bpf_end_states.rds")
} else {
...
The results of these projections are summarized in Figs.~\ref{fig:Mod1Scenarios}--\ref{fig:elimProbs}, and suggest that cholera elimination was likely even without increased vaccination efforts---consistent with observed reality \citep{trevisin22}.
...
Notably, the median number of cumulative cholera infections under the no-vaccination scenario using Models 1 and 3 were $\Sexpr{prettyNum(round(as.numeric(h1_S0_results$mod1_V0_probs$cumInf['q50'])), big.mark = ",")}$ and $\Sexpr{prettyNum(round(as.numeric(mod3_V0_res$mod3_V0_probs$cumInf['q50'])), big.mark = ",")}$, respectively.
...
\subsection{Corroborating Fitted Models with Previous Scientific Knowledge}\label{sec:science}

The resulting mechanisms in a fitted model can be compared to current scientific knowledge about a system.
Agreement between model based inference and our current understanding of a system may taken as a confirmation of both model based conclusions and our scientific understanding.
...
%% The potential for new discoveries based on fitting a dynamic model, however, is rooted in a model's ability to effectively approximate the system in question, and is therefore inexorably linked to a model's goodness-of-fit.
%% As such, conclusions based on dynamic models that poorly fit observed data should not be considered as much as those resulting from models that describe observed data well. THIS DUPLICATES LATER SENTENCES, SO I THINK IT CAN BE REMOVED HERE.
...
Specifically, we examine the results of fitting the flexible cubic spline term in Model~1 (Eq.~\myeqref{model1:lambda}--\myeqref{model1:beta}).
...
This is consistent with previous studies finding that rainfall played an important role in cholera transmission in Haiti \citep{lemaitre19,eisenberg13}.
...
Under the framework of Model~2, the best estimate for this parameter is $\Sexpr{signif(1/h2_fit$h2_params["sigma"],3)}$, suggesting that individuals have effectively permanent immunity to cholera once infected, and thus the pool of susceptible individuals decreases over time.
To interpret this result, we bear in mind that the data ranged from 2010-2019, and therefore estimates of immunity longer than 10 years---the upper end of previous estimates of natural immunity \citep{king08}---effectively result in the same model dynamics.
...
\section{Robust interpretation of model based conclusions}\label{sec:advice}
...
In addition, parameters that have been calibrated to data should make sense in the context of alternative lines of evidence about the phenomena being modeled, while making allowance for the possibility that the interpretations of parameters may vary when modeling across differing spatial scales.
...
Improved model fits allows for meaningful statistical inference that may provide valuable insight on a dynamic system and may improve the accuracy of model forecasts.
...
In this article we argue that careful attention to important statistical details could have led the models proposed by \cite{lee20} to correctly predict the imminent cholera elimination.
...
% We acknowledge there are limitations to this study, such as the inability to fit model parameters to the fully-coupled version of Model~3.
...
\sdescription{
This document contains additional details for Models~1--3, as well as a translation table that facilitates comparisons between these models and those described by \cite{lee20}.
...
\sdescription{
This \code{R} package is contained in a public GitHub repository: jeswheel/haitipkg. The package contains all of the data and code used to create and fit the models, as well as other useful functions that were used in this article.
}
\end{supplement}

\begin{supplement}
\stitle{\code{jeswheel/haiti\_article}}
\sdescription{
