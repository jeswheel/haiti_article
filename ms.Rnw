...
Cholera was considered eliminated in 2019, though it reemerged in 2022.
...
Together, with the density for the initial value of the latent state $\bm{X}^{(\modelCounter)}_{0} = \bm{X}^{(\modelCounter)}(t_0)$, each model assumes a joint density $f_{\bm{X}^{(\modelCounter)}_{0:N}, \bm{Y}^{(\modelCounter)}_{1:N}}\big(\bm{x}^{(\modelCounter)}_{0:N}, \bm{y}^{(\modelCounter)}_{1:N};\paramVec\big)$, where $\paramVec$ is a parameter vector that indexes the model. \arc{(Perhaps have this next sentence later when $U$ has been defined?)}
...
Specifically, we set $I_{u0}(0) = \frac{y^*_{1u}}{\reportRate(\muDeath + \choleraDeath + \muIR)}$, $A_{u0}(0) = \frac{1 - \symptomFrac}{\symptomFrac}I_{u0}(0)$, $R_{u0k} = y^*_{1u} - I_{u0}(0) - A_{u0}(0)$, and initialize $B_{u}(0)$ \arc{(define $B_{u}$?)} by enforcing the rainfall dynamics supposed by the one step transition model;
...
To do this, we perform a profile-likelihood search on the parameter $\transmissionTrend$ and obtain a confidence interval \arc{(confidence level?)} via a Monte Carlo Adjusted Profile (MCAP) \citep{ionides17}.
...
The blue curve is the profile, the blue line indicates the MLE, and the dashed lines indicate the confidence interval. \arc{level?}
}
\end{figure}
...
The use of benchmarks may also be beneficial when developing models at differing spatial scales, where a direct comparison between model\ar{s} likelihoods is meaningless.
...
For frequentist inference, one possible approach is obtaining model forecasts from various values of $\theta$, where the values of $\theta$ are sampled proportionally according to their corresponding likelihoods \citep{king15}; see Sec.~S7 of the supplement material for more details.
...
Improved model fits allow for meaningful statistical inference that may provide valuable insight on a dynamic system and may improve the accuracy of model forecasts.
...
Robust data analysis requires not only reproducibility but also extendability: if one wishes to try new model variations, or new approaches to fitting the existing models, or plotting the results in a different way, this should \ar{be not} not be excessively burdensome.
