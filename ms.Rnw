...
\new{Despite their importance, creating realistic, data-informed models that can be used to reliably and accurately influence policy is a challenging task.
In this article, we demonstrate that increasing the standards of mathematical modeling of ecological system can improve our ability to use models to understand the dynamics of a complex, non-linear system, and make data informed inferences on such systems.
...
Here, we suggest that this can be done by comparing fitted models to a suitable statistical benchmark, and verifying that model outcomes---including estimated model parameters, latent states, and seasonal trends---make sense in the context of the system in question.}

\new{In the context of our cholera case study, we demonstrate the importance of model diagnostics by using them to improve model fits, resulting in more reliable conclusions.
...
In the case of Model 3, the process of comparing a fitted model to a benchmark and investigating why the model did not describe the data as well as a simple linear benchmark}
