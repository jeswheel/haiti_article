...
We focus on Models~1--3, as the strengths and weaknesses of the agent-based modeling approach  \citep{tracy18} are outside the scope of this article, and we focus on Models~1--3.
...
Although several cholera chases have recently been detected in Haiti (\citep{TODO}),
their conclusion is inconsistent with the prolonged period with no confirmed cholera cases between February, 2019 \citep{trevisin22} and \jwc{TODO}.

The discrepancy between the model based conclusions of \cite{lee20} and the prolonged absence of cholera in Haiti has been debated \citep{francois20,rebaudetComment20,henrys20,leeReply20}.
...
\\[-3pt]
\label{eq:model3:IS}
\mu_{I_{u\vaccCounter}S_{u0}} &=& \muDeath + \choleraDeath, \hspace{2mm} \mu_{A_{u\vaccCounter}S_{u0}} = \muDeath
\\[-3pt]
...
All deaths are balanced by births into the susceptible compartment in \myeqref{eq:model3:IS} and \myeqref{eq:model3:RS}, thereby maintaining constant population in each department.
...
shared_param_names <- c(
  ...
  "epsilon", "k", "sigma", "rho", "gamma"
)

mod3_n_params <- length(unit_specific_names) * 10 + length(shared_param_names)
...
set_rw_1 <- function(x) {
  if (grepl('^foi_add[[:digit:]]+$', x) || grepl('^betaB[[:digit:]]+$', x)) {
    return(0.01 / 2)
  } else if (grepl('^Binit[[:digit:]]+$', x)) {
    return(expression(ivp(0.025)))
  } else {
    return(0.01)
  }
}
...
# Third Search RW
reg_rw_3.sd <- lapply(est_param_names_expanded, set_rw_3)
...
However, the congruency between the model and observed data serves as a proxy for the congruency between the model and the true underlying dynamic system, since the observed data are a result of the generating system.
As such, the best possible fit of the proposed mechanistic structure to the observed data is desirable.
In the following subsections we describe our approach to calibrating the three proposed mechanistic models to observed cholera incidence data.
...
Several algorithms exist, both frequentist and Bayesian, that can be used to obtain estimates of the parameters for this class of models.
...
To our knowledge, the only plug-and-play, frequentist methods that can maximize the likelihood for POMP models of this complexity are iterated filtering algorithms \citep{ionides15}, which modify the well-known particle filter \citep{arulampalam02} by performing a random walk for each parameter and particle.
...
Because the process model of Model~2 is deterministic, the parameter estimation problem for Model~2 reduces to a least squares calculation when combined with a Gaussian measurement model (see supplement material for details) \jwc{TODO: cite which part of supplement}.
...
One issue that arises when fitting spatially explicit models is that parameter estimation techniques based on the particle filter become computationally intractable as the number of spatial units increase.
...
The simplified, spatially-decoupled version of Model~3 implemented by \cite{lee20} relies on the observed cholera cases for each department to describe cholera dynamics, making it impossible to obtain forecasts for future time points from the fitted model.
Therefore, in order to obtain model forecasts, the parameters of the calibrated conditionally independent department level models were used in the fully coupled version of Model~3.
...
if (RUN_LEVEL != 3 & file.exists("model3/run_level_3/h3Sims.rds")) {
  h3_quants <- readRDS("model3/run_level_3/h3Sims.rds")
} else {
  h3_quants <- bake(
...
\jwc{TODO: CHECK} It should be universal practice to present measures of goodness of fit for published models, and mechanistic models should be compared against benchmarks.
...
if (RUN_LEVEL != 3 & file.exists("model3/run_level_3/mod3_eval_res.rds")) {
  mod3_eval_res <- readRDS("model3/run_level_3/mod3_eval_res.rds")
} else{
  mod3_eval_res <- bake(
...
if (RUN_LEVEL != 3 & file.exists("model3/run_level_3/lee3_results.rds")) {
  ...
  lee3_results <- bake(
    file = paste0('model3/', rl_dir, 'lee3_results.rds'), {
      ...
      lee3_results[["n_params"]] <- 29  # TODO: Check to make sure this is the correct number.
...
Values in parenthesis are corresponding values using \citet{lee20} parameter estimates.  $\textsuperscript{1}$The reported likelihood is an upper bound of the likelihood of the \citet{lee20} model. $\textsuperscript{2}$\cite{lee20} fit Model~3 to a subset of the data (March 2014 onward, excluding data from Ouest in 2015-2016).
...
See Sec.~S5 of the supplement material for more details on estimating the likelihood of the \cite{lee20} models.
}
\end{table}

Table~\ref{tab:likes} shows that despite the significant improvements that were made by using a contemporary approach of parameter calibration for Model~3, this model still fails to beat a log-linear ARMA benchmark.
This suggests that Model~3 is failing to adequately explain some substantial aspect of the data.
As our goal for this article is the estimation and diagnosis of the models at hand, we explore the shortcoming in log-likelihood further in section \jwc{TODO} of the supplement material.
There, we use additional tools like conditional log-likelihoods to understand how well the model describes each data point, given the previous observations.
Additional tools for assessing the goodness-of-fit of a model include plotting the effective sample size of each observation \citep{liu01}, and comparing summary statistics of the observed data to simulations from the model \citep{wood10}; these summary statistics are sometimes called diagnostic probes \citep{king15,king16}.
...
For frequentist inference, one possible approach obtaining model forecasts from various values of $\theta$, and then weighing the resulting forecasts by their corresponding log-likelihoods \citep{king15}; see section \jwc{TODO} of the supplement material for more details.
...
Infection (day)
  ...
  & \eqref{model2:mu_IR}
  & {\fixed $\muIR^{-1}=\Sexpr{myround(365/p3s["gamma"],1)}$ }
  & \eqref{eq:model3:IR}
\\
Latency (day)
...
This article demonstrates that fitting appropriate scientific models remains a challenging statistical task, and therefore great care is needed when fitting scientific models with the purpose of making policy recommendations.
...
In this article we argue that careful attention to important statistical details could have led the models proposed by \cite{lee20} to correctly predict the initial elimination of cholera from Haiti in 2019.
...
At a minimum, we have shown that the conclusions are sensitive to details in how the data analysis is carried out, and that attention to statistical fit (including numerical consideration such as likelihood maximization) can lead to improved policy guidance.
...
\sdescription{
This document contains additional details about the numerical implementations of Models~1--3, as well as a translation table that facilitates comparisons between these models and those described by \cite{lee20}.
The supplement also demonstrates our capability to faithfully estimate the likelihood of the models of \cite{lee20}.
}
\end{supplement}
