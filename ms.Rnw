...
Under the framework of Model~2, the best estimate for this parameter is $\Sexpr{signif(1/h2_fit$h2_params["sigma"],3)}$ yr, suggesting that individuals have \old{effectively} permanent immunity to cholera once infected.
...
\new{We presented methodology consistent with existing guidelines but going beyond standard practice.
In particular, we showed the value of comparing the likelihood of fitted mechanistic models versus non-mechanistic benchmarks.
Further, we used likelihood-based diagnostic methods to identify specific limitations of proposed models.
We used likelihood profile methods to assess parameter identifiability and to construct confidence intervals for parameter estimates.
