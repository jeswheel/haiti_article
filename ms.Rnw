...
The observed data $\bm{y}_{1:N}^*$ are modeled as a realization of the observation processes. \jwc{SHOULD WE SAY SOMETHING ABOUT HOW THE JOINT DENSITY IS INSTRACTABLE, OR CAN BE FACTORED USING THE MARKOV PROPERTY?}
...
Model~2 is a system of deterministic differential equations and can therefore be considered a degenerate probabilistic model with no randomness in the dynamic process.
By combining a deterministic process model with a simple Gaussian measurement model, the parameter estimation problem for a set of differential equations reduces to a least squares calculation.
