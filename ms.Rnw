...
We further demonstrate the utility of recent advances in likelihood maximization for high-dimensional nonlinear dynamic models, enabling likelihood-based inference for high-dimensional models for spatiotemporal incidence data.}
...
Our analysis presents methodology for diagnosing how well a model describes observed data\new{.}\old{, and provides careful justification of valid conclusions from the fitted model.}
Objective measures are used to \old{benchmark} \new{evaluate} model fit; when \old{these}\new{objective measures of the goodness-of-fit of a model} are combined with reproducibility, a framework emerges for continual improvement \new{to model based inference} when revisiting the data\old{ and models}.
\new{This framework promotes scientific discovery and bolsters model-based policy recommendations by simplifying the process for researchers to build off of previous results.}
...
\new{They also provided a range for the forecasted number of cumulative cases from February 2019 to Februaruy 2024 based on model simulations, with median estimates from each model measured in hundreds of thousands.}
...
mod3_eval_res <- bake(
  file = paste0('model3/', rl_dir, 'mod3_eval_res.rds'), {
    ...
    u_cond_ll <- t(sapply(h3_bpf, function(x) rowSums(x@block.cond.loglik)))
    results$u_cond_ll <- apply(u_cond_ll, 2, logmeanexp, se = TRUE)
...
Here, $\vaccCounter=0$ corresponds to unvaccinated individuals, and $\vaccCounter \in \seq{1}{\vaccClass}$ describes hypothetical vaccination programs\new{, where each program $\vaccCounter$ indexes differences in both the number of doses administered (one versus two doses per individual) and the round of vaccine administration.}
...
Therefore, for departments with zero case counts in week 1, we \old{consider estimating}\new{estimate} the number of infected individuals rather then treating this value as a constant (see supplement Sec.~S5 for more details).
...
This results in confounded mechanisms, as one may not formally test all possible ways to include a trending parameter. In such a case, t}\old{T}he robust statistical conclusion is that a model which allows for change fits better than one which does not.\old{---we argue that a decreasing transmission rate is a plausible way to explain this, but} \new{Because} the incidence data themselves \old{do} \new{may} not provide enough information to pin down the mechanism\new{, a strongly supported conclusion should avoid ruling out untested hypotheses.}
\new{In this case study, we did not explicitly test if the incidence data favor a trend in transmission over a trend in reporting rate. We argue that a decreasing transmission rate is a plausible way to explain the decrease in cases over time, as there is alternative evidence that supports this model \cite{rebaudet19CATI,rebaudet21,michel19}. Additionally, there is little scientific evidence to support a decrease in reporting rate (for example, see Figure~1 of \cite{rebaudet21}).}
...
\new{Models that are fit to the same datasets can be directly compared using AIC values, making it a useful tool to compare to benchmark models. Though Models~2 and 3 are both fit to department level incidence reports, their AIC values are not directly comparable due to the way Model~3 initializes latent states, which is described in detail in Sec. S5 of the supplement.}
...
\new{Rather than interpreting this as scientific evidence that individuals have permanent immunity from cholera, this result suggests that Model~2 favors a regime where reinfection events are a negligble part of the dynamics.}
