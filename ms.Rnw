...
An important example is the marginal density of the observed random vector $\bm{Y}_{1:N}$, evaluated at the observed data $\bm{y}_{1:N}^*$: $f_{\bm{Y}_{1:N}}\big(\bm{y}_{1:N}^*; \paramVec\big) = \int f_{\bm{X}_{0:N}, \bm{Y}_{1:N}}\big(\bm{x}_{0:N}, \bm{y}_{1:N};\paramVec\big) d\bm{x}_{0:N}$.
When treated as a function of the parameter vector $\paramVec$, this is called the \emph{likelihood function}, which is the basis of likelihood based statistical inference.

Using the conditional independence of $\bm{Y}_{1:N}$ given $\bm{X}_{0:N}$ and the Markov property of $\bm{X}_{0:N}$, the joint density can be refactored as: $f_{\bm{X}_{0:N}, \bm{Y}_{1:N}}\big(\bm{x}_{0:N}, \bm{y}_{1:N};\paramVec\big) = f_{\bm{X}_0}\big(\bm{x}_0;\paramVec\big)\prod_{n = 1}^N f_{\bm{X}_n|\bm{X}_{n-1}}\big(\bm{x}_{n}|\bm{x}_{n-1}; \paramVec\big)f_{\bm{Y}_n|\bm{X}_{n}}\big(\bm{y}_n|\bm{x}_{n}\big)$.
...
For any time point in $t_{1:N}$, the process model describes the movement of individuals from one latent state compartment to another.
...
In the case of a POMP model, the likelihood function---as described in Sec.~\ref{sec:models}---is written as $\mathcal{L}(\paramVec) = \int f_{\bm{X}_{0:N}, \bm{Y}_{1:N}}\big(\bm{x}_{0:N}, \bm{y}_{1:N};\paramVec\big) d\bm{x}_{0:N}$.
...
To our knowledge, the only plug-and-play methods that can maximize the likelihood for POMP models of this complexity are iterated filtering algorithms \citep{ionides15}, which modify the well-known \emph{particle filter} \citep{arulampalam02}.
...
Iterated filtering algorithms, like IF2 \citep{ionides15}, extend the particle filter by performing a random walk for each parameter and particle;
