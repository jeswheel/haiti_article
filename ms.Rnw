...
<<LoadModel1Params, echo=FALSE, message=FALSE, include=FALSE>>=
...
<<FitModel2, echo=FALSE, include=FALSE, message=FALSE, warning=FALSE>>=
...
<<createModelObjects,echo=FALSE,eval=TRUE, include=FALSE, message=FALSE>>=
...
<<AICtableInput, echo=FALSE, eval=TRUE>>=
...
<<CalculateBenchLikelihood, echo=FALSE, message=FALSE, include=FALSE, cache=TRUE>>=
...
rm(cases, m3_benchmark, m3_bench, dep, mod3_cases)
gc()
@

<<AIC Lee et al, echo=FALSE, include=FALSE, message=FALSE>>=
...
<<cleanUpBetatFig, echo=FALSE, message=FALSE, include=FALSE, warning=FALSE>>=
suppressWarnings(
...
h3_bpf_end_stats <- readRDS("model3/run_level_3/h3_bpf_end_states.rds")
