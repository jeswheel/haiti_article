...
\new{This framework promotes scientific discovery and supports model-driven policy recommendations by simplifying the process of understanding and improving previous results.}
...
Human to water
  & \missing
  &
  & $\Wshed = $ \Sexpr{signif(h2_fit$h2_params["Mu"]/52,3)}
  & \eqref{model2:to_W}
...
\end{equation}
\new{where $\bar{\transmission} = 1 (\mathrm{wk})^{-1}$ is a dimensionality constant.}
...
In this article, we demonstrate opportunities available for raising the current standards of statistical inference for mathematical models of biological systems.}

\new{We present methodology consistent with existing guidelines but exceeding standard practice.
