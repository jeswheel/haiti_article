...
<<FitModel1, echo=FALSE, message=FALSE>>=
set.seed(636813)
...
We then fit the spatially decoupled version of Model~3 using a slight modification of the PIF algorithm, which we call the Block Panel Iterated Filter (BPIF).
...
While the reported parameters were obtained using the decoupled representation of Model~3, simulations for the various vaccination campaigns were obtained using fully coupled version of the model.
...
We note, however, that $\bm{X}^{(\modelCounter)}_N$ is unobservable and therefore draws from the desired conditional density are unobtainable.
...
\subsection{Comparison of Model Fit to Oustide Scientific Evidence}\label{sec:science}
...
This result provides evidence that rainfall plays an important role in cholera transmission in Haiti, which is consistent with previous studies \citep{eisenberg13,rinaldo12}.
We also note the increase in the transmission rate during the Fall, which was also noticed at an earlier stage of the epidemic \citep{rinaldo12}.
...
\subsection{Predicting the Effect of Interventions}\label{sec:interventions}
