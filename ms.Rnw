...
\title{Informing policy via dynamic models: Cholera in Haiti}
\runtitle{Cholera in Haiti}
...
The 2010 introduction of cholera to Haiti led to an extensive outbreak and sustained transmission.
Cholera was eliminated in 2019, though it returned in 2022.
...
Their conclusion is inconsistent with the prolonged period with no confirmed cholera cases between February, 2019 and September, 2022 \citep{trevisin22} though cholera has recently reemerged in Haiti \citep{rubin22}.
...
Additional details about the numeric implementation of these models are provided in Secs.~S1 and S2 of the supplement.
...
See Sec.~\ref{sec:fit2} and Sec.~S3.2 of the supplement for more details.
...
Per-capita transition rates are given in Equations~\ref{eq:model3:SI}--\ref{eq:model3:Decay}.
\begin{eqnarray}
\label{eq:model3:SI}
...
As such, for departments with zero case counts in week 1, we consider estimating the number of infected individuals rather then treating this value as a constant (see supplement Sec.~S5 for more details).
Finally, reported cholera cases are assumed to come from a negative binomial measurement model with mean equal to a fraction ($\reportRate$) of individuals in each unit who develop symptoms seek healthcare
...
Because the process model of Model~2 is deterministic, the parameter estimation problem for Model~2 reduces to a least squares calculation when combined with a Gaussian measurement model (see Sec.~S3.2 of the supplement material for details)
...
Model~3 describes cholera dynamics in Haiti using a metapopulation model, where the hidden states in each administrative department has an effect on the dymanics in other departments.
...
% and fitting using the IF2 algorithm to estimate parameters separately for independent department-level models;
...
Because the simplified, spatially-decoupled version of Model~3 implemented by \cite{lee20} relies on the observed cholera cases it is impossible to obtain forecasts for future time points from the fitted model.
Therefore, in order to obtain model forecasts, \cite{lee20} used the parameters estimates from the spatially-decoupled approximation of Model~3 to obtain forecasts using the fully coupled version of the model.
...
as set of parameters that maximize the likelihood of the decoupled model almost certainly do not maximize the likelihood of the fully coupled model.
...
\hline
\end{tabular}
\caption{\label{tab:likes}AIC values for each models compared to their ARMA benchmarks.
...
While a useful practice, comparing model log-likelihoods to a suitable benchmark alone may not be sufficient to diagnosing the goodness-of-fit of a given model.
...
Other tools include plotting the effective sample size of each observation \citep{liu01}; plotting the values of the hidden states from simulations (supplement Sec.~5.1);
...
For frequentist inference, one possible approach obtaining model forecasts from various values of $\theta$, where the values of $\theta$ are sampled proportionally their corresponding likelihoods \citep{king15}; see Sec.~S7 of the supplement material for more details.
...
In this analysis, our forecasts do not account for parameter uncertainty as our goal is to comment on the estimation and diagnosis of mechanistic models, rather than suggesting our results to be used to influence vaccination policy in Haiti.
Furthermore, we use the projections from a single point estimate to highlight a deficiency of deterministic models, which is that the only variability in model projections is a result of parameter uncertainty, which leads to over-confidence in forecasts \cite{king15}.
...
\multirow{2}{*}{Seasonality}
  ...
  & $\transmissionTrend = \Sexpr{myround(p1["betat"],2)}$ [N]
  & \eqref{model1:betat}
  & $\phaseParm = \Sexpr{myround(h2_fit$h2_params['phase'], 2)}$ [N]
  & \eqref{model2:lambda}
...
\end{array}$
  & $\sigmaProc=(\Sexpr{myround(sqrt(p1["sig_sq_epi"]),2)}, \Sexpr{myround(sqrt(p1["sig_sq_end"]),2)})$ [N]
  & \eqref{model1:lambda}
  & \multicolumn{1}{c}{\hspace{16pt}\missing}
  &
...
\multirow{2}{*}{Initial Values} & $I_{0,0} = \Sexpr{round(prod(p1[c('pop_0', 'I_0')]))}$ & & \multicolumn{1}{c}{\multirow{2}{*}{\hspace{16pt}\missing}} & & \multirow{2}{*}{$I_{3,7,10;0}(0)= (
        \Sexpr{round(prod(p3[c('H3', 'Iinit3')]))}, \Sexpr{round(prod(p3[c('H7', 'Iinit7')]))}, \Sexpr{round(prod(p3[c('H10', 'Iinit10')]))})$ [N]} & \\
& $E_{0,0} = \Sexpr{round(prod(p1[c('pop_0', 'E_0')]))}$ & & \multicolumn{1}{c}{} & & & \\\hline
Hurricane & \multicolumn{1}{c}{\multirow{2}{*}{\hspace{16pt}\missing}} & & \multicolumn{1}{c}{\multirow{2}{*}{\hspace{16pt}\missing}} & & $\Whur{3, 9} = (\Sexpr{myround(p3['aHur3'], 2)}, \Sexpr{myround(p3['aHur9'], 2)})$ [N] & \multirow{2}{*}{\eqref{eq:model3:foi}} \\
Parameters & \multicolumn{1}{c}{} & & \multicolumn{1}{c}{} & & $\hHur{3, 9} = (\Sexpr{myround(p3['hHur3'], 2)}, \Sexpr{myround(p3['hHur9'], 2)})$ [N] & \\\hline
...
The package contains all of the data and code used to create, fit and explore the models presented in this article.
}
\end{supplement}
