...
In this article, we conduct a re-analysis of the 2010 cholera outbreak in Haiti.
\new{We use this case study to investigate current procedures for fitting mathematical models to time series data, while identifying limitations of these methodologies and proposing remedies.}
We present methodology for diagnosing how well a model describes observed data\new{.}\old{, and provides careful justification of valid conclusions from the fitted model.}
...
\new{Confidence intervals for model parameters are given in the supplement (Sec.~S8).}
