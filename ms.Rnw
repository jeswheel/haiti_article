...
# Create vectors for the unit and shared parameters that are estimated
unit_specific_names <- c("betaB", "foi_add", "aHur", "hHur", "Iinit")
shared_param_names <- c(
  "mu_B", "XthetaA", "thetaI", "lambdaR", "r", "std_W", "epsilon", "k"
)

est_param_names <- c(unit_specific_names, shared_param_names)
...
# Simple function that is used to set the rw_sd for the second search
# This function is convenient so that we don't have to type out the rw_sd
# for each unit parameter.
set_rw_2 <- function(x) {
  if (gsub("[[:digit:]]+$", "", x) %in% shared_param_names) {
...
# Simple function that is used to set the rw_sd for the third search
# This function is convenient so that we don't have to type out the rw_sd
# for each unit parameter.
set_rw_3 <- function(x) {
  if (gsub("[[:digit:]]+$", "", x) %in% shared_param_names) {
...
haiti3_fit <- bake(
  file = paste0("model3/", rl_dir, "haiti3_coupled_fit.rds"), {
    fit_coupled_haiti3(
      nsearches = h3_fit_parms$N_SEARCHES,
      search1 = h3_fit_parms$SEARCH1,
      search2 = h3_fit_parms$SEARCH2,
      search3 = h3_fit_parms$SEARCH3,
      search_rho = FALSE,
      search_gamma = FALSE,
      search_Iinit = TRUE,
      search_hur = TRUE,
