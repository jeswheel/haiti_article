...
#
#                    RUN_LEVEL =  1,   2,    3
N_PARAMS   <- switch(RUN_LEVEL,   5,  20,  nrow(h1LeeStartsEpi))  # Number of parameters to sample
N_LEE_SIMS <- switch(RUN_LEVEL,   5,  20,   20)  # Number of simulations per parameter
NP_EVAL    <- switch(RUN_LEVEL,  50, 200, 2000)  # Number of particles used to evaluate the likelihood
NREPS_EVAL <- switch(RUN_LEVEL,   2,   5,   36)  # Number of replicates of likelihood evaluation.

h1_epi <- haiti1()

set.seed(141837)

if (RUN_LEVEL == 1 && file.exists('model1/run_level_3/lee1_epi_evals.rds')) {

  # If RUN_LEVEL == 1, and RUN_LEVEL == 3 results exist, just load the RL3
  # results.
  lee_epi_ll <- readRDS("model1/run_level_3/lee1_epi_evals.rds")
} else {

  # Compute the run_level_<RUN_LEVEL> results and save them, or simply load
  # the results if the bake load conditions are met.
  lee_epi_ll <- bake(
...
if (RUN_LEVEL == 1 && file.exists('model1/run_level_3/lee1_end_evals.rds')) {
...
\eic{I'VE RE-WRITTEN HERE, LOOKING TO BE MORE SPECIFIC ABOUT THE GOALS AND CONTRIBUTION OF THE PAPER.}
...
Specifically, we present approaches \old{to diagnosis of model misspecification, development of alternative models, and computational improvements in optimization, in the context of likelihood-based inference on nonlinear dynamic systems.} \new{of diagnosing model misspecification, which facilitates the development of model improvements.
We further demonstrate the usefulness of recent advancements in likelihood optimization for high-dimensional nonlinear dynamic models, resulting in increased concurrence between high dimensional models and observed incidence data.}
...
In this article, we discuss common modeling decisions that may not provide an adequate statistical explanation of the data, and we show that attention to statistical fit can lead to more accurate policy \old{evaluations} \new{recommendations}.
We do this by conducting a re-analysis of the 2010 cholera outbreak in Haiti.
Our analysis presents methodology for diagnosing how well a model describes observed data, and provides careful justification of valid conclusions from the fitted model.
Objective measures are used to benchmark model fit; when these are combined with reproducibility, a framework emerges for continual improvement when revisiting the data and models.
...
\TODO{I think we can and should directly state something here that we discuss with the reviewers: the article has both educational purposes, arguing how a data analysis using dynamic models should happen, and as an in-depth analysis of cholera in Haiti.}
\new{This analysis serves both as a pedagogical...}
...
\new{Various dynamic models were developed to study this outbreak: searching PubMed with keywords ``Haiti, cholera, model'' we obtained 20 deterministic models \cite{kirpich16,kirpich17,andrews11,botelho21,fitzgibbon20,eisenberg13,rinaldo12,chao11,date11,lin19,collins21,akman15,trevisin22,mavian20,collins14,kelly16,capone15,bengtsson15,mari15,gatto12} and 8 stochastic models \cite{tuite11,mukandavire13,lewnard16,kunkel17,sallah17,azman12,kuhn14,azman15}.
...
\new{These modeling strategies were selected by \cite{li20} to represent the range of approaches used in the research community.}
\jwc{I think the citation above is incorrect? Should it be lee20, not li20?}.
...
\new{In addition, agent-based models were less widely used, the agent based model of \cite{lee20} being the only model of this class that was found in our literature review.}
\new{The data that were used for this study, and that is the subject of our reanalysis, are displayed in Fig.~\ref{fig:CholeraData}.}
...
The groups largely adhered to existing guidelines for creating models to inform policy \cite{behrend20,saltelli20} and, despite their autonomy, obtained a consensus that an extensive nationwide vaccination campaign would be necessary to eliminate cholera from Haiti. \TODO{How? This statement should be epanded or re-evaluated.}
...
Though cholera has recently reemerged in Haiti \cite{rubin22,PAHO23}, the inability to accurately forecast cholera incidence from 2019-2022---while diligently adhering to existing scientific standards for infectious disease modeling---demonstrates that existing standards of disease modeling and forecasting may be insufficient.
...
\new{Specifically, we focus on how to develop and test variations to these proposed models, as well as diagnosing the models once they have been assimilated to incidence reports.} \jwc{make a statement about fitting Model 3 (stochastic meta-pop) here?}
...
# Load the file containing profiles of Model 1 Parameters.
if (RUN_LEVEL != 3 && file.exists("model1/run_level_3/h1_profiles.rds")) {
  h1_prof <- readRDS("model1/run_level_3/h1_profiles.rds")
} else if (file.exists(paste0('model1/', rl_dir, 'h1_profiles.rds'))) {
  h1_prof <- readRDS(paste0('model1/', rl_dir, 'h1_profiles.rds'))
} else {
  stop(paste0("Missing file: `model1/", rl_dir, 'h1_profiles.rds`;\nThe file can be obtained by running the script `H1profile.R`'))
}
...
# TODO: This is used later in figure 2 (TODO)
...
lee20_replication <- bake(
  file = 'model2/lee20_replication.rds', {
    h2_lee_epi <- haiti2()
    lee_epi_params <- h2_lee_epi@params
    epi_ofun_v <- traj_objfun(
      h2_lee_epi, est = c('v'), paramnames = c('v')
    )

    # Get starting values for the estimated parameter
    start_v_epi <- lee_epi_params[c('v')]

    # Use the subplex package to calibrate 'v' for the epi-model with fixed coefs.
    epi_fit_v <- subplex::subplex(par = log(start_v_epi), fn = epi_ofun_v)
    epi_params_update_v <- lee_epi_params
    epi_params_update_v['v'] <- exp(epi_fit_v$par)

    # Now we repeat this for the endemic portion of the model:
    h2_lee_end <- haiti2(region = 'after')
    lee_end_params <- h2_lee_end@params
    end_ofun_v <- traj_objfun(
      h2_lee_end, est = 'v', paramnames = 'v'
    )

    # Get starting values for the estimated parameter
    start_v_end <- lee_end_params[c('v')]

    # Use the subplex package to calibrate 'v' for the epi-model with fixed coefs.
    end_fit_v <- subplex::subplex(par = log(start_v_end), fn = end_ofun_v)
    end_params_update_v <- lee_end_params
    end_params_update_v['v'] <- exp(epi_fit_v$par)

    # Return the results
    c(
      "leeFit_ll" = -epi_fit_v$value - end_fit_v$value,
      "leeFit_n_params" = 6
    )
  }, timing = FALSE
)
...
h2_fit <- bake(
  file = 'model2/model2_fit.rds', {
    ...
    theta <- h2_params[c("Mu", "Beta", "BetaW", "v", "sigma")]
    ...
    h2_params[c("Mu","Beta","BetaW","v", "sigma")] <- exp(h2_fit_tmp$par[c("Mu", "Beta", "BetaW", "v", "sigma")])
    h2_params['phase'] <- h2_fit_tmp$par['phase']
    ...
    rm(h2, epi_ofun, theta, h2_fit_tmp)
...
# Load the file containing profiles of Model 1 Parameters.
if (RUN_LEVEL != 3 && file.exists("model3/run_level_3/h3_profiles.rds")) {

  # If RUN_LEVEL != 3, and RUN_LEVEL == 3 results exist, just load the RL3
  # results.
  h3_prof <- readRDS("model3/run_level_3/h3_profiles.rds")
} else if (file.exists(paste0('model3/', rl_dir, 'h3_profiles.rds'))) {

  # If the `model3/run_level_<RUN_LEVEL>/h3_profiles.rds` file exists, load it
  h3_prof <- readRDS(paste0('model3/', rl_dir, 'h3_profiles.rds'))
} else {

  # Otherwise (missing RL3 file, and the corresponding <RUN_LEVEL> file):
  # Throw an error, letting users know how to create the file.
  stop(paste0("Missing file: `model3/", rl_dir, 'h3_profiles.rds`;\nThe file can be obtained by running the script `H3profile.R`'))
}
...
NP_EVAL    <- switch(RUN_LEVEL,  50, 500,  5000)
...
mod3_eval_res <- bake(
  file = paste0('model3/', rl_dir, 'mod3_eval_res.rds'), {
    ...
    ll <- logLik(h3_bpf)
    results$ll <- logmeanexp(ll[!is.na(ll)], se = TRUE)
    ...
    results$subset_ll <- logmeanexp(mod3_coupled_subset_evals, se = TRUE)
    results
  },
  timing = FALSE
)
...
<<Calculate Bench Likelihood, echo=FALSE, message=FALSE, include=FALSE, cache=TRUE>>=
...
\jwc{Maybe right here is another place for the new paragraph on mechanistic models?}
...
\begin{tabular}{|p{0.15\linewidth}|p{0.14\linewidth}t|p{0.13\linewidth}t|p{0.17\linewidth}t|}
\hline
...
\\
Asympt.
  & ${\asymptomRelativeInfect=0.05}\fixed$
  & \eqref{model1:lambda}
...
\multirow{4}{*}{Human to human}
  ...
  & \multirow{4}{*}{$\transmission=$\Sexpr{signif(h2_fit$h2_params["Beta"],3)}}
...
\multirow{5}{*}{Water to human}
  ...
  & $\Wbeta{_{1:10}}= (
        ...
        \Sexpr{myround(p3u["betaB",10],2)})$
  & \multirow{4}{*}{\eqref{eq:model3:foi}}
\\
& & & \multirow{-3}{*}{$\beta_W= \Sexpr{signif(h2_fit$h2_params["BetaW"],3)}$} & \multirow{-3}{*}{\eqref{model2:lambda}} & &
\\
\hline
Human to water
  & \missing
...
\raggedright Water survival (wk)
  & \multirow{2}{*}{\missing}
  &
  & \multirow{2}{*}{${\Wremoval^{-1} = \Sexpr{signif(52/h2_fit$h2_params["Delta"],3)}}\fixed$}
  & \multirow{2}{*}{\eqref{model2:from_W}}
  & \multirow{2}{*}{$\Wremoval^{-1}=\Sexpr{myround(52/p3s["mu_B"],2)}$}
  & \multirow{2}{*}{\eqref{eq:model3:Decay}}
\\
Mixing exponent
...
\raggedright Process noise (wk\textsuperscript{1/2})
  & \raggedright $\sigmaProc = (\Sexpr{myround(sqrt(p1["sig_sq_epi"]),2)},
  \Sexpr{myround(sqrt(p1["sig_sq_end"]),2)})^*$
  & \multirow{2}{*}{\eqref{model1:lambda}}
  & \multirow{2}{*}{\missing}
  &
  & \multirow{2}{*}{$\sigmaProc= \Sexpr{myround(p3s["std_W"],3)}$}
...
More details about the initialization model $f_{\bm{X}_0}\big(\bm{x}_0;\paramVec\big)$ and the \emph{measurement model} $f_{\bm{Y}_n|\bm{X}_{n}}\big(\bm{y}_n|\bm{x}_{n}\big)$ for Models~1--3 are provided a supplement text (\nameref{S_init} and \nameref{S_meas}).
...
Finally, reported cholera cases are modeled using a negative binomial measurement model with mean equal to a fraction ($\reportRate$) of individuals in each unit who develop symptoms and seek healthcare
...
Calibrating model parameters to observed data does not guarantee that the resulting model successfully approximates real-world mechanisms, since the model description of the dynamic system may be incorrect and do not change as the model is calibrated to data.
...
\eic{I'VE STARTED TO PUT IN SOME DISCUSSION OF BAYESIAN METHODS HERE, THOUGH IT COULD ALSO GO IN THE DISCUSSION}
\new{The main alternative to maximum likelihood estimation is Bayesian inference via Markov chain Monte Carlo, used to analyze the Haiti cholera epidemic by \cite{andrews11,rinaldo12,lewnard16,trevisin22,sallah17,azman12,kuhn14,mari15,gatto12}.
Alternatively, some investigations obtained parameters from previous sources without calibrating to data.}
...
\end{tabular}
\begin{flushleft}
Values in parentheses are corresponding values using Lee et al.~\cite{lee20} parameter estimates.  \textsuperscript{1}The reported likelihood is an upper bound of the likelihood of the Lee et al.~\cite{lee20} model. \textsuperscript{2}Lee et al.~\cite{lee20} fit Model~3 to a subset of the data (March 2014 onward, excluding data from Ouest in 2015-2016).
...
Details of estimating the likelihood of the Lee et al.~\cite{lee20} models is provided in \nameref{S_lee20}
See Sec.~S6 of the supplement material for more details on estimating the likelihood of the Lee et al.~\cite{lee20} models.
\end{flushleft}
\end{table}

\subsubsection*{Calibrating Model~1 Parameters}\label{sec:fit1}

Model~1 was developed with the intent of providing a description of cholera dynamics that is scientifically plausible rather than one that is statistically convenient.
This results in the inability to obtain a closed form expression of the non-linear joint model density---described in Eq.~\myeqref{eq:jointLik}.
...
% In particular, a closed form expression of the joint model density---described in Eq.~\myeqref{eq:jointLik}---is not readily available, making likelihood based inference a challenging task.
% In the case of any POMP model, the likelihood function---as described in Eq.~\myeqref{eq:likedef}---is written as $\mathcal{L}(\paramVec) = \int f_{\bm{X}_{0:N}, \bm{Y}_{1:N}}\big(\bm{x}_{0:N}, \bm{y}_{1:N};\paramVec\big) d\bm{x}_{0:N}$.
% The high-dimensional integral in the likelihood, and the potentially non-linear nature of the process model, make it impossible to obtain a closed form expression of the likelihood function of POMP models except in the most simple cases.
% Model~1 was developed with the intent of providing a description of cholera dynamics that is scientifically plausible rather than on that is simply statistically convenient.
...
The resulting 95\% confidence interval for $\transmissionTrend$ is $(\Sexpr{myround(mcap_results$ci[1], digits = 3)}, \Sexpr{myround(mcap_results$ci[2], digits = 3)})$, with the full results displayed in Fig.~\ref{fig:betat}.
...
\new{The maximum likelihood estimate for $\transmissionTrend$ corresponds with a \Sexpr{paste0(myround(100 - 100*exp(mcap_results$mle * 430/215), 1), '\\%')} reduction to the transmission rate over the course of the outbreak, with a $95\%$ confidence interval (\Sexpr{paste0(rev(myround(100 - 100*exp(mcap_results$ci * 430/215), 1)), collapse = ', ', sep = '\\%')}) for the overall reduction in transmission.}
...
\new{This example demonstrates the type of model variation that should be considered in the course of a data analysis: the model variation---the addition of a trend in transmission rate---has a plausible scientific justification, and is easily testable using likelihood based inference methods and observed data.
This class of model variations are commonly considered in epidemiological modeling \cite{lemaitre19,fitzgibbon20,eisenberg13,rinaldo12,kunkel17}} , \new{but their adaptation are sometimes based on subjective assessments \cite{fitzgibbon20,eisenberg13,kunkel17}} \TODO{get citations}.
\jwc{Alternatively, we could emphasize the importance of considering model variations, and cite studies from the literature review that only considered a single model, or only reported their final chosen model. Although models that were not ultimately selected are not as important, a discussion of which models you tried and why you didn't use them can be beneficial to future researchers who may want to consider similar variations in the future. I got to article 35 checking for those with multiple models considered.}
...
The statistical evidence of a trend in transmission rate in this model could be explained by any trending variable (such as hygiene improvements, or changes in population behavior), resulting in confounding from collinear covariates.
\jwc{HERE'S SOME INITIAL THOUGHTS ON HOW TO CLARIFY THIS FOR THE REVIEWER}.
\old{Alternatively, the trend could be attributed to a decreasing reporting rate rather than decreasing transmission rate, resulting in confounded mechanisms.}
\new{Because we only have access to incidence data, the decrease in observed cases over the course of the outbreak may alternatively be attributed to a trend in other model parameters, such as a decrease in the reporting rate over time.
This results in confounded mechanisms, as likelihood-based comparisons of a model that permits a change in reporting rate against a model that permits a change in transmission rate may not yield definitive results.}
The robust statistical conclusion is that a model which allows for change fits better than one which does not\old{---we argue that a decreasing transmission rate is a plausible way to explain this}, but the incidence data themselves \old{do} \new{may} not provide enough information to pin down the mechanism.
\new{In this case study, we argue that a decreasing transmission rate is a plausible way to explain the decrease in cases over time \cite{rebaudet19CATI,rebaudet21,michel19}; there is little scientific evidence to support a decrease in reporting rate, as the rate of testing increased throughout this time period.} \TODO{cite: Wasn't there a study we thought demonstrated the testing effort increased}?
...
These two concerns may explain the unrealistic forecasts and low likelihood of Model~3 of Lee et al.~\cite{lee20} (see Table~\ref{tab:likes}).
...
\jwc{Should there be more added / said to this paragraph, to demonstrate the novelty of being able to fit a model like Model 3? Maybe add a paragraph between the first and second paragraphs of this sub-section.}
...
\jwc{Should we make benchmark modeling a sub-section of model-diagnostics? I think this could (A) add some organization to the paper so that readers / reviewers can clearly see how benchmarks help answer the question we posed: what indications should we look for in data to assess trustworthiness of data (B) further emphasize the importance of benchmarks, which is something the reviewers seemed to have found useful. }
...
We chose to use the auto regressive negative binomial model, however, because the large number of weeks with zero recorded cholera cases in department level data makes a benchmark based on a continuous distribution problematic.
...
In the context of our case study, by reconstructing the latent states of Model~3, we notice that the calibrated model favors higher levels of cholera transmission than what was typically observed in the incidence data (\nameref{S_mod3cal}).
...
We obtain forecasts by simulating future values using latent states that are consistent with the most recent observations; this is done by drawing latent states at the last observation time ($t_N$) from the filtering distribution $f_{\bm{X}_N|\bm{Y}_{1:N}}(\bm{x}_{N} | \bm{y}^*_{1:N} ; \hat\paramVec)$.
The decision not to do this partially explains the unsuccessful forecasts of Lee et al.~\cite{lee20}: their Table~S7 shows that the subset of their simulations which were consistent with observing zero cases in 2019 also accurately predicted the prolonged absence of detected cholera.
...
Compare to Fig.~3A of Lee et al.~\cite{lee20}.}
\end{figure}
...
The ongoing global COVID-19 pandemic has provided a clear example on how government policy may be affected by the conclusions of \old{scientific} \new{mathematical} models \cite{saltelli20}.
...
\new{Despite their importance, creating scientifically plausible, data-informed models that can be used to reliably gain insight into a dynamic system is a challenging task.
In this article, we demonstrate that increasing the standards used for the mathematical modeling of ecological systems can improve our ability to use models to understand the dynamics of a complex, non-linear system, and make data informed inferences.
We suggest a greater effort be placed on evaluating dynamic models that will be used to make inference on a system.
Specifically, we argue that researchers can and should look for indications in available data in order to assess whether model based inferences are trustworthy.
Here, we show that this can be done by comparing fitted models to a suitable statistical benchmark, and verifying that model outcomes---including estimated model parameters, latent states, and seasonal trends---make sense in the context of the system in question.}

\new{In the cholera case study, we demonstrate the importance of model diagnostics by using them to improve model fits, resulting in more reliable conclusions.
We compared the Models 1--3 of \cite{lee20} to simple statistical benchmarks, which enabled us to find deficiencies in the models that could be improved.
In the case of Model~3, the process of comparing a fitted model to a benchmark and investigating why the model did not describe the data as well as its benchmark helped us recognize that the model could not adequately explain the increase in cholera transmission that resulted from Hurricane Matthew, leading us to propose a model alteration.
Because Model~1 outperformed its benchmark model, we were able to confidently inspect implications from the fitted model; here we found that the seasonality term of Model~1 closely follows seasonal rainfall patterns in Haiti, both confirming previously held beliefs about the importance of rainfall as a driver of cholera infection, as well as bolstering our confidence that Model~1 captures important dynamics of the cholera outbreak.}

\new{In any study using dynamic models, more than one model framework---or model variation---should be considered, tested, and reported.
In our literature review of modeling cholera in Haiti, only a few papers that used mechanistic models reported more than a single model variation. (TODO: Cite).
We recognize the possibility that a study that reports only a single model may have considered others and reported only the model that they used for the final analysis; details about models that were tried but not used can be of great benefit to the scientific community, however, as they may help future researchers to avoid frameworks that have previously been shown to be less favorable.}

\new{While likelihood based inference enables researchers to chose between any model that is evaluated on the same dataset using a metric like AIC, nested model variations are particularly useful as they enable formal statistical testing of the nested features.
In the cholera case study, we demonstrate the benefit of this type of model variation in each of the three models.
In Model~1, we tested the inclusion of a trend in transmission rate over time; in Model~2, we considered adding an additional phase parameter, which enables a shift in the seasonal peaks of cholera infections; in Model~3, we included additional parameters such as the Hurricane Matthew related parameters.
In each of these cases, we found strong statistical evidence that favored these model variations over their more simple counterparts.}

\new{Another important consideration when fitting a mechanistic model to a dynamic system is the complexity of the model.
Mathematical models necessarily simplify complex dynamical systems in order to make inference possible.
Decisions about which aspects of a system to include in a model and which to exclude can have significant impacts on model based conclusions, but can be tested under a likelihood-based framework.
Common modeling decisions include deterministic vs stochastic modeling, and whether or not to include a meta-population structure.
Model benchmarks can be useful in determining the level of modeling complexity to use, allowing researchers to calibrate the relative performance of a model of any given complexity against a better understood statistical model.}

\new{The three models in this case study each represent a unique combination of decisions of model complexity.
Model~3 provides an interesting example in that it is both stochastic and has a meta-population structure. This class of models is notoriously difficult to perform likelihood based inference on (TODO: Cite).
Here we demonstrate how this class of model can be calibrated to incidence data using the novel IBPF algorithm.
This case study demonstrates the potential benefit of this algorithm, as it is one of the examples that uses this algorithm.
}
...
\new{Specifically, each of the re-calibrated models predict that cholera would disappear from Haiti with moderate probably.
While new cases of cholera have since been detected in Haiti, this conclusion is more consistent with the prolonged absence of cholera cases that occurred from 2019-2022.}
