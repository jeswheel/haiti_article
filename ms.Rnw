...
The search resulted in 66 papers, of which 32 proposed or used dynamic models to describe the cholera epidemic in Haiti.
This literature review confirmed that close attention to details in fitting, diagnosing, and forecasting is of great importance when modeling cholera in Haiti.
...
Additional tools for assessing the goodness-of-fit of a model include plotting the effective sample size of each observation, and comparing any statistic of the observed data to simulations from the model---which is sometimes referred to as diagnostic probes.
...
If a Bayesian technique is used for parameter estimation, the most natural way to account for parameter uncertainty is to obtain multiple simulations from the model where each simulation is obtained using parameters $\paramVec$ drawn from the posterior distribution $\paramVec \sim f_{\Theta}\big(\paramVec | \bm{Y}^{(\modelCounter)}_{1:N} = y_{1:N}^*\big)$.
In the frequentist case, one approach is a quasi-empirical Bayes: endow to set $\Theta_K = \{\theta_1, \theta_2, \ldots, \theta_K\}$ with a discrete uniform distribution, such that $P\big(\paramVec = \theta_k\big) = \frac{1}{K}$ for all values $k \in \seq{1}{K}$.
In this case, the posterior distribution of $\paramVec | \bm{Y}^{(\modelCounter)}_{1:N}$ can be expressed as: 
$P\big(\paramVec = \theta_k | \bm{Y}^{(\modelCounter)}_{1:N} = y_{1:N}^*\big) = \frac{l(y_{1:N}^*; \theta_k)}{\sum_{j = 1}^K l(y_{1:N}^*; \theta_j)}$; to complete the formulation, we then choose the set $\Theta_K$ such that each $\theta_k \in \Theta_K$ is a single output of the MIF2 algorithm, which has been shown to be within a neighborhood of the MLE \cite{ionides15}. 
This approach has been taken in previous studies \citep{king15}, and the result is simply weighing parameter sets based on their corresponding likelihoods: parameters with higher likelihoods should be used more frequently in model forecasts. 
...
Agreement between model based inference and our current understanding of a system may taken as a confirmation of both model based conclusions and our understanding of the system.
...
After fitting the model parameters, we explore potential patterns in the seasonal transmission rate by plotting the average value of $\transmission$ in a typical year.
...
\section{Robustness of model based conclusions}\label{sec:advice}
