...
\end{equation}
where $\tHur$ is the time Hurricane Matthew struck Haiti \jwc{TODO Cite}, and $1_{(A)}$ is the indicator function for event $A$.
In \cite{lee20}, $\Whur{_u}$ and $\hHur{u}$ were set to zero for all $u$;
the need to account for the effect Hurricane Matthew had on cholera transmission for this model is explored in Sec. \jwc{TODO} of the supplement.
...
As such, for departments with zero case counts in week 1, we consider estimating the number of infected individuals rather then treating this value as a constant (see supplement Sec. \jwc{TODO} for more details).
...
(see Sec. \jwc{TODO} of the supplement).
...
\jwc{TODO: cite which part of supplement}.
...
These comparisons, which provide insight into how each model performs relative to the others, are quite common \jwc{CITE SOME HAITI PAPERS}.
...
Other tools include plotting the effective sample size of each observation \citep{liu01}; plotting the values of the hidden states from simulations (see supplement Sec.\jwc{TODO});
...
For frequentist inference, one possible approach obtaining model forecasts from various values of $\theta$, and then weighing the resulting forecasts by their corresponding log-likelihoods \citep{king15}; see Sec. \jwc{TODO} of the supplement material for more details.
