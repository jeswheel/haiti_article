...
\new{In this article, we conduct a re-analysis of the 2010 cholera outbreak in Haiti. We use this case study to investigate current procedures for fitting mathematical models to time series data, while identifying limitations of these methodologies and proposing remedies.}
\new{We present} \old{Our analysis presents} methodology for diagnosing how well a model describes observed data\new{.}\old{, and provides careful justification of valid conclusions from the fitted model.}
...
\new{However, the development of credible models---which are supported by data and can lend quantitative insights into a dynamic system---remains a challenging task.
In this article, we demonstrate opportunites available for raising the current standards of statistical inference for mathematical models of biological systems.

We present methodology consistent with existing guidelines but exceeding standard practice.  
In particular, we show the value of comparing the likelihood of fitted mechanistic models versus non-mechanistic benchmarks.
Further, we use likelihood-based diagnostic methods to identify specific limitations of proposed models.
We use likelihood profile methods to assess parameter identifiability and to construct confidence intervals for parameter estimates. 
We assess the accuracy of numerical approximations involved in our calculations.
When reaching conclusions, we identify potential consequences of confounded variables and model misspecification.
}
