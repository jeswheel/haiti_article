...
While this work is focused on the models proposed in \cite{lee20}, we believe that our suggestions are relevant to a larger class of problems.
In order to determine the extent to which this work generalizes, we preformed a literature review by searching PubMed with keywords: Haiti, cholera, and model.
The search resulted in 66 papers, of which 32 used dynamic models to describe the cholera epidemic in Haiti.
This literature review confirmed that close attention to details in fitting, diagnosing, and forecasting is of great importance when modeling an infectious disease outbreak.
% Although the literature review focused on cholera and Haiti,
The wide relevance of our findings in these papers---supplemented with studies of other epidemiological and dynamic systems---suggests that this work may also be relevant to a much broader class of problems.
...
Model~1 is implemented using the \code{pomp} package \cite{pomp2}, relying heavily on the source code provided by \cite{lee20}, who also used the MIF2 algorithm to estimate $\paramVec$.
Aside from the introduction of the parameter $\transmissionTrend$ and the multiplicative Gamma-distributed white noise $d\Gamma(t)/dt$ in the process model (Eq.~\ref{model1:lambda}), the only change made that could increase the likelihood of the model was using a substantially increased computational effort to maximize the likelihood.
While flexible, plug-and-play methods like MIF2 require a large number of computations, and the theoretic ability to maximize the likelihood depends on asymptotics in both the number of particles used and the number of MIF iterations.
...
Model~2 is a deterministic model, which are degenerate probabilistic models that contain no randomness in the dynamic process or measurement.
...
In practice, however, the inclusion of overdispersion in the measurement process may be preferred.
