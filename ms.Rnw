...
stew(
  file = "model2/output/model2_fit.rda", {h2_fit <- fit_haiti2()}
)

# load('model2/output/model2_fit_params.rda')
# p2_epi <- h2_epi_params
# h2_fit$end_params <- h2_end_params
# rm(h2_epi_params, h2_end_params)
# gc()
...
stew(
  file = "model3/output/sims_FinalPanel.rda", {
    ...
    h3_panel_quantiles <- h3_panel_all_sims %>%
    ...
    h3_panel_real_data <- h3_panel_quantiles %>%
      filter(isdata)

    h3_panel_sim_data <- h3_panel_quantiles %>%
      filter(!isdata)

  }
)

h3_panel_sim_data <- h3_panel_sim_data %>%
  mutate(
    dep = gsub("-", "_", dep),
    date = yearsToDateTime(time)
  ) %>%
  filter(date >= yearsToDate(h3@unit.objects$Artibonite@t0)) %>%
  mutate(date = as.Date(lubridate::round_date(date)))

h3_panel_real_data <- h3_panel_real_data %>%
  mutate(
    dep = gsub("-", "_", dep),
    date = yearsToDateTime(time)
  ) %>%
...
ggplot() +
  geom_line(data = h3_panel_sim_data, aes(x = date, y = q50 + 1), col = 'blue') +
  geom_line(data = h3_panel_real_data, aes(x = date, y = q50 + 1), col = 'black') +
  geom_ribbon(data = h3_panel_sim_data, aes(x = date, ymin = q05 + 1, ymax = q95 + 1),
              alpha = 0.4) +
...
rm(h3_panel_all_sims, h3_panel_sim_data)
gc()
@
...
registerDoRNG(6687147)

stew(
  file = "model3/output/SpatPompSims.rda", {
    h3Spat_sims <- simulate(h3Spat, nsim = 500, seed = 784713,
                     format = "data.frame", include.data = TRUE)

    h3Spat_quants <- h3Spat_sims %>%
      mutate(is.data = .id == 'data') %>%
      filter(!is.data) %>%
      rename(dep = unitname) %>%
      group_by(dep, time) %>%
      summarise(q05 = quantile(cases, 0.025, na.rm = T),
                mean = mean(cases, na.rm = T),
                q50 = quantile(cases, 0.5, na.rm = T),
                q95 = quantile(cases, 0.975, na.rm = T)) %>%
      ungroup() %>%
      mutate(date = yearsToDateTime(time)) %>%
      filter(date >= yearsToDate(h3Spat@t0)) %>%
      mutate(date = as.Date(lubridate::round_date(date)))

    h3Spat_truth <- h3Spat_sims %>%
      # select(time, .id, starts_with("cases")) %>%
      mutate(is.data = .id == 'data') %>%
      filter(is.data) %>%
      rename(dep = unitname) %>%
      group_by(dep, time) %>%
...
ggplot() +
  ...
  geom_line(data = h3Spat_truth, aes(x = date, y = q50 + 1), col = 'black') +
...
rm(h3Spat_sims, h3Spat_truth, h3Spat_quants)
gc()
@
...
load('model3/output/FinalPanelAsSpatPomp_PF.rda')
rm(pf, ll)
mod3_spatPomp_ll <- ll_est[1]
rm(ll_est)
mod3_spatPomp_n_params <- 39
mod3_spatPomp_aic <- 2 * mod3_spatPomp_n_params - 2 * mod3_spatPomp_ll
@
...
stew(
  # Save the file
  file = 'model3/output/lee3_results.rda', {
    ...
    lee3_results[["sub_pf_time"]] <- system.time({
      ...
      lee3_results[["sub_ll_evals"]] <- foreach(
        ...
        logLik(pfilter(lee3_sub, Np = 5000))  # Get likelihood evaluations
      }
    })
    ...
    lee3_results[["all_pf_time"]] <- system.time({
      ...
      lee3_results[["all_ll_evals"]] <- foreach(
        ...
        logLik(pfilter(lee3_all, Np = 5000))  # Get likelihood evaluations
      }
    })
...
rm(lee3_sub, lee3_all)
gc()
@


<<old TODO: Replace this with the above>>=
# Load Lee et al Model 3 likelihood for entire data
# load('model3/output/lee_mod3_coupled_allData_ll.rda')
# mod3_lee_coupled_ll <- max(lee3_results[["ll_evals_sub_data"]])
# mod3_lee_coupled_n_params <- 29
mod3_lee_coupled_aic <- 2 * lee3_results$n_params - 2 * max(lee3_results$all_ll_evals)

# rm(coupled_ll_evals_all_data, coupled_ll_results_all_data, tPfilter_lee_coupled)
# gc()

# Load Lee et al Model 3 likelihood on partial data
# load('model3/output/lee_mod3_coupled_ll.rda')
# mod3_lee_coupled_partial_ll <- max(coupled_ll_evals_sub_data)
mod3_lee_coupled_partial_aic <- 2 * lee3_results$n_params - 2 * max(lee3_results$sub_ll_evals)

# Load our spatPOMP Model 3 on likelihood on partial data
load('model3/output/mod3_coupled_subset_ll.rda')
mod3_coupled_subset_aic <- 2 * mod3_spatPomp_n_params - 2 * mod3_coupled_subset_ll
@
...
\multirow{2}{*}{Log Likelihood} &
  ...
  $\Sexpr{myround(mod3_spatPomp_ll, 1)}$ \\
...
On this same subset of data, our model achieved a likelihood of $\Sexpr{myround(mod3_coupled_subset_ll, 1)}$.}
...
% Model projections given in the following section have been obtained using the technique suggested in this subsection.
% Sentence above is unecessary - Jesse
...
load('model1/vacscens/sims_S0.rda')
mod1_V0_sims <- agg_mod1_sims(sims)
mod1_V0_probs <- get_elimProbs(sims, model = 1)
rm(sims)
gc()

load('model1/vacscens/sims_S2.rda')
mod1_V1_sims <- agg_mod1_sims(sims)
mod1_V1_probs <- get_elimProbs(sims, model = 1)
rm(sims)
gc()

load('model1/vacscens/sims_S4.rda')
mod1_V2_sims <- agg_mod1_sims(sims)
mod1_V2_probs <- get_elimProbs(sims, model = 1)
rm(sims)
gc()

load('model1/vacscens/sims_S3.rda')
mod1_V3_sims <- agg_mod1_sims(sims)
mod1_V3_probs <- get_elimProbs(sims, model = 1)
rm(sims)
gc()

load('model1/vacscens/sims_S1.rda')
mod1_V4_sims <- agg_mod1_sims(sims)
mod1_V4_probs <- get_elimProbs(sims, model = 1)
rm(sims)
gc()
@
...
stew(
  file = "model2/output/VaccinationScenarios.rda", {
    mod2_VaccScenarios <- haiti2_vaccScenario(
...
load('model3/output/sims_spatPomp_S0.rda')
mod3_V0_sims <- agg_mod3_sims(sims)
mod3_V0_probs <- get_elimProbs(sims, model = 3)
rm(sims)
gc()

load('model3/output/sims_spatPomp_S2.rda')
mod3_V1_sims <- agg_mod3_sims(sims)
mod3_V1_probs <- get_elimProbs(sims, model = 3)
rm(sims)
gc()

load('model3/output/sims_spatPomp_S4.rda')
mod3_V2_sims <- agg_mod3_sims(sims)
mod3_V2_probs <- get_elimProbs(sims, model = 3)
rm(sims)
gc()

load('model3/output/sims_spatPomp_S3.rda')
mod3_V3_sims <- agg_mod3_sims(sims)
mod3_V3_probs <- get_elimProbs(sims, model = 3)
rm(sims)
gc()

load('model3/output/sims_spatPomp_S1.rda')
mod3_V4_sims <- agg_mod3_sims(sims)
mod3_V4_probs <- get_elimProbs(sims, model = 3)
rm(sims)
gc()
@
...
Notably, the median number of cumulative cholera infections under the no-vaccination scenario using Models 1 and 3 were $\Sexpr{prettyNum(round(as.numeric(mod1_V0_probs$cumInf['q50'])), big.mark = ",")}$ and $\Sexpr{prettyNum(round(as.numeric(mod3_V0_probs$cumInf['q50'])), big.mark = ",")}$, respectively.
...
gg_m3_V0 <- ggplot() +
  geom_line(data = mod3_V0_sims, aes(x = date, y = q50 + 1), col = 'blue') +
  geom_ribbon(data = mod3_V0_sims, aes(x = date, ymin = q05 + 1, ymax = q95 + 1), alpha = 0.5) +
  geom_line(data = true_agg_cases, aes(x = date, y = ReportedAll + 1)) +
  ylab("Log reported cases") +
  theme(axis.title.x = element_blank()) +
  ggtitle("V0") +
...
gg_m3_V1 <- ggplot() +
  geom_line(data = mod3_V1_sims, aes(x = date, y = q50 + 1), col = 'blue') +
  geom_ribbon(data = mod3_V1_sims, aes(x = date, ymin = q05 + 1, ymax = q95 + 1), alpha = 0.5) +
  geom_line(data = true_agg_cases, aes(x = date, y = ReportedAll + 1)) +
  ylab("Log reported cases") +
  theme(axis.title = element_blank(),
        axis.ticks.y = element_blank(),
...
gg_m3_V2 <- ggplot() +
  geom_line(data = mod3_V2_sims, aes(x = date, y = q50 + 1), col = 'blue') +
  geom_ribbon(data = mod3_V2_sims, aes(x = date, ymin = q05 + 1, ymax = q95 + 1), alpha = 0.5) +
  geom_line(data = true_agg_cases, aes(x = date, y = ReportedAll + 1)) +
  ylab("Log reported cases") +
    theme(axis.title = element_blank(),
        axis.ticks.y = element_blank(),
...
gg_m3_V3 <- ggplot() +
  geom_line(data = mod3_V3_sims, aes(x = date, y = q50 + 1), col = 'blue') +
  geom_ribbon(data = mod3_V3_sims, aes(x = date, ymin = q05 + 1, ymax = q95 + 1), alpha = 0.5) +
  geom_line(data = true_agg_cases, aes(x = date, y = ReportedAll + 1)) +
  ylab("Log reported cases") +
    theme(axis.title = element_blank(),
        axis.ticks.y = element_blank(),
...
gg_m3_V4 <- ggplot() +
  geom_line(data = mod3_V4_sims, aes(x = date, y = q50 + 1), col = 'blue') +
  geom_ribbon(data = mod3_V4_sims, aes(x = date, ymin = q05 + 1, ymax = q95 + 1), alpha = 0.5) +
  geom_line(data = true_agg_cases, aes(x = date, y = ReportedAll + 1)) +
  ylab("Log reported cases") +
    theme(axis.title = element_blank(),
        axis.ticks.y = element_blank(),
...
mod3_V0_probs$ElimTime$scenario = "V0"
mod3_V1_probs$ElimTime$scenario = "V1"
mod3_V2_probs$ElimTime$scenario = "V2"
mod3_V3_probs$ElimTime$scenario = "V3"
mod3_V4_probs$ElimTime$scenario = "V4"

mod3_probElims <- rbind(
  mod3_V0_probs$ElimTime,
  mod3_V1_probs$ElimTime,
  mod3_V2_probs$ElimTime,
  mod3_V3_probs$ElimTime,
  mod3_V4_probs$ElimTime
)
