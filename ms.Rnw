...
Because our primary focus is on the statistical tasks of fitting and diagnosing mechanistic models, we refrain from commenting on the feasibility of scientific assumptions implicitly made in \cite{lee20}.
We demonstrate that model-mispecification does not alone explain the discrepancy between model predictions and reality, as has been suggested \citep{rebaudetComment20}.
...
Ultimately, these decisions will result in a different approach to calibrating (or fitting) model parameters.
...
One approach to modelling a dynamic system is through probabilistic models.
...
Plug-and-play algorithms include ABC and PMCMC \citep{toni09,andrieu10}, but here we focus frequentist methods to maximize model likelihoods.
...
The ability to obtain MLEs for each of the model parameters does not immediately imply that the model itself
...
The purpose of parameter calibration, irrespective of the approach taken (e.g., frequentist versus Bayesian), is to ensure that the proposed model provides the best possible description of the observed data under the assumptions of the model, as measured by some metric (such as model likelihoods).
...
In fact, we echo the sentiment of George Box, to whom the aphorism ``all models are wrong, but some are useful" is often attributed; here, we assume that that some degree of model mispecification is always present, and therefore a researcher must subjectively determine whether or not the model mispecification is severe enough to render the model ``useless".

% Researchers may use fitted models to make inference and draw conclusions about the dynamic system under investigation.
% Because of the casual nature of mechanistic models, the validity of conclusions drawn using a fitted model is dependent on the ability of the proposed model to approximate the system in question.
It is often impossible to know, however, the extent to which a model provides a faithful representation of the system.
...
In such comparisons, these simulations are random draws from the complete estimated joint distribution $f_{\bm{X}^{(\modelCounter)}_{0:N}, \bm{Y}^{(\modelCounter)}_{1:N}}\left(x_{0:N}, y_{1:N}\mid \hat{\theta}\right)$, where $\modelCounter$ indexes the model used for simulations.
...
In this case, draws from the conditional density $f_{\bm{X}^{(\modelCounter)}_{N:s}, \bm{Y}^{\modelCounter}_{N:s}}\big(x_{N:s}, y_{N:s}\mid \hat{\theta}, \bm{X}^{(\modelCounter)}_{N} = \bm{x}^{(\modelCounter)}_N\big)$ should be preferred, as these simulations account for the most recent known state of the system.

$\bm{X}^{(\modelCounter)}_N$, however, is unobservable and therefore draws from the desired conditional density are unobtainable.
Informed estimates of $\bm{X}^{(\modelCounter)}_N$ given the observed data $\bm{Y}^{(\modelCounter)}_{1:N} = y_{1:N}^*$ and the proposed model can easily be obtained, however, via the filtering distribution.
...
A single model forecast can then be obtained by simulating from the model $f_{\bm{X}^{(\modelCounter)}_{N:s}, \bm{Y}^{(\modelCounter)}_{N:s}}\big(x_{N:s}, y_{N:s}\mid \hat{\theta}, \bm{X}^{(\modelCounter)}_{N} = \hat{\bm{X}}^{(\modelCounter), i}_N\big)$.
...
We note---the credit of deterministic models---that of the four fitted models in \cite{lee20}, Model~2 provides what seems to be the most accurate model projections.
...
An additional benefit of mechanistic modelling is ability to simulate various interventions on a system; this feature is useful to inform policy and was the primary goal of \citet{lee20}.
...
Mimicking their efforts, we define cholera elimination as having less than one infection of cholera over at least 52 consecutive weeks in the 10-year projection period, and provide projections under the following vaccination scenarios:
