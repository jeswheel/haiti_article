...
While this work is focused on the models proposed in \cite{lee20}, our suggestions have broader relevance.
...
\subsubsection{Model~2}\label{sec:fit2}
...
In the frequentist case, one approach is empirical Bayes, as was done in \cite{king15} (see supplement for details).
...
In this article we argue that careful attention to important statistical details could have led the models in \cite{lee20} to correctly predict the imminent cholera elimination.
