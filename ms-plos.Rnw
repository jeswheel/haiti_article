...
Scientists often use data from an infectious disease outbreak to build statistical models.
Among other things, these models are often used to guide governmental policy, as the models can provide insight into the dynamics of an outbreak.
Despite recent computational and methodological advancements, fitting dynamic models to observed incidence data remains a challenging task.
In this article, we discuss common mistakes that may lead to models that do not adequately describe reality, and make suggestions that can lead to more useful statistical models.
We do this by conducting a re-analysis of the 2012-2019 cholera outbreak in Haiti.
We focus on the importance of appropriately diagnosing how well a model describes observed data, correctly interpreting model based results, and stressing the importance of reproducibility.
An important contribution of this article is demonstrating a use-case of a newly developed computational algorithm to fit spatially coupled meta-population models.
We assert that raising the currently accepted standards of infectious disease modeling will result in a greater ability of scientists and policy makers to understand and respond to future infectious disease outbreaks.
...
The data that was used for this study, and that is the subject of our reanalysis, is displayed in Fig.~\ref{fig:CholeraData}.
...
# by hand.
m2_inits <- c(6449, 542, 0, 0, 16, 0, 0, 522, 0, 0)  # TODO: These were for the original initialization model, may need to fit these again by hand (just for-loop through possible inits.)
...
m3_inits <- c(7362, 1914, 22, 8, 2818, 82, 2499, 6953, 44, 7) # TODO: Also fit by hand, but could be changed to initializing to the observation before.
...
Translations back to the original notation of Lee et al.~\cite{lee20} are given in Table~S-1 of the supplement. % TODO: Check S-1 table matches.
...
\multirow{3}{*}{Seasonality}
  ...
  & \eqref{model1:beta}
  & \multirow{2}{*}{${\fixed \seasAmplitude=0.4}$}
  ...
  & \multirow{3}{*}{${\fixed \muRS^{-1}=\Sexpr{myround(1/p1["alpha"]/52,1)}}$}
  & \multirow{3}{*}{\eqref{model1:RS}}
  & $\muRS^{-1}= \Sexpr{signif(1/h2_fit$h2_params["sigma"],2)}$
  & \eqref{model2:RS}
  & \multirow{3}{*}{${\fixed \muRS^{-1}=\Sexpr{myround(1/p3['rho1'],1)}}$}
  & \multirow{3}{*}{\eqref{eq:model3:RRnext}}
\\
& & & ${\fixed \omega_1^{-1} = \Sexpr{myround(1/h2_fit$h2_params["Omega1"],1)}}$ & \eqref{model2:omega1} & &\\
& & & ${\fixed \omega_2^{-1} = \Sexpr{myround(1/h2_fit$h2_params["Omega2"],1)}}$ & \eqref{model2:omega2} & &\\
\hline
\multirow{2}{*}{Birth/death (yr)}
  & ${\fixed \muBirth^{-1} = \Sexpr{myround(1/p1["mu"]/52,1)}}$
    %
  & \eqref{model1:death}
  & \multirow{2}{*}{\missing}
  &
  & \multirow{2}{*}{${\fixed \muDeath^{-1} = \Sexpr{myround(1/p3["mu1"],1)}}$}
  & \multirow{2}{*}{\eqref{eq:model3:IS}}
\\
& ${\fixed \muDeath^{-1} = \Sexpr{myround(1/p1["delta"]/52,1)}}$
& \eqref{model1:death}
&
&
& &
...
\\
Asympt.
  & \multirow{2}{*}{${\fixed \asymptomRelativeInfect=0.05}$}
...
\multirow{5}{*}{Water to human}
  ...
  % & \multicolumn{1}{c}{\hspace{16pt}\missing}
  &
  & \multirow{3}{*}{${\fixed \Wsat = \Sexpr{signif(h2_fit$h2_params["Sat"],2)}}$}
...
Water survival \newline (wk)
  ...
  % & \multicolumn{1}{c}{\hspace{16pt}\missing}
  &
  & \multirow{2}{*}{${\fixed \Wremoval^{-1} = \Sexpr{signif(52/h2_fit$h2_params["Delta"],3)}}$}
...
\\
\hline
Process noise \newline (wk\textsuperscript{1/2})
  & $\sigmaProc =$ \newline $(\Sexpr{myround(sqrt(p1["sig_sq_epi"]),2)},
...
\\
Observation \newline variance
  & $\obsOverdispersion =$ \newline $(\Sexpr{myround(p1["tau_epi"],2)}, \Sexpr{round(p1["tau_end"],2)})$
  & \multirow{2}{*}{(S15)} % TODO: Check
  & \multirow{2}{*}{$\obsOverdispersion = \Sexpr{signif(h2_fit$h2_params['sigma'], 3)}$}
  & \multirow{2}{*}{(S16)} % TODO: Check
  & \multirow{2}{*}{$\obsOverdispersion = \Sexpr{myround(p3s["k"],2)}$}
  & \multirow{2}{*}{(S19)} % TODO: Check
\\
\hline
\multirow{2}{*}{Initial Values} & $\Iinit = \Sexpr{round(prod(p1[c('pop_0', 'I_0')]))}$
  &
  & \multirow{2}{*}{\missing}
  % & \multicolumn{1}{c}{\multirow{2}{*}{\hspace{16pt}\missing}}
  &
  & \multirow{2}{*}{$\Iinit^{3,7,10} = (
        \Sexpr{round(prod(p3[c('H3', 'Iinit3')]))}, \Sexpr{round(prod(p3[c('H7', 'Iinit7')]))}, \Sexpr{round(prod(p3[c('H10', 'Iinit10')]))})^*$} % TODO: Check the unit numbers
  &  % TODO: \multirow{2}{*}{EQUATION NUMBER}
  \\
...
% & \multicolumn{1}{c}{\multirow{2}{*}{\hspace{16pt}\missing}}
  ...
  % & \multicolumn{1}{c}{\multirow{2}{*}{\hspace{16pt}\missing}}
  &
  & $\Whur{3, 9} =$\newline $(\Sexpr{myround(p3['aHur3'], 2)}, \Sexpr{myround(p3['aHur9'], 2)})^*$
...
\\
 \multirow{-1}{*}{Parameters} &
  ...
  % & \multicolumn{1}{c}{}
  &
  & $\hHur{3, 9} =$\newline $(\Sexpr{myround(p3['hHur3'], 2)}, \Sexpr{myround(p3['hHur9'], 2)})^*$
  & \multirow{2}{*}{\eqref{eq:model3:foi}}
\\\hline
\end{tabular}
\caption{\label{tab:params}References to the relevant equation are given in parentheses.
...
\hline
\end{tabular}
\caption{\label{tab:likes}AIC values for each model compared to their respective benchmarks.
...
The process model that defines Model~1 was developed with the intent of providing a plausible description of cholera dynamics rather than a providing statistically convenient description.
...
{\figTitle Confidence inverval for linear trend in transmission.}
...
Projections under the various vaccination scenarios, which are discussed in the \bf{Forecasts} subsection are also included.}
\end{figure}
...
These two concerns may explain the unrealistic forecasts and low likelihood of Model~3 from Lee et al.~\cite{lee20} (see Table~\ref{tab:likes}).

At the time Lee et al.~\cite{lee20} was published, there was no known algorithm that could readily be used to maximize the likelihood of an arbitrary meta-population POMP model with coupled spatial dynamics, which justifies the spatial decoupling approximation that was used to calibrate model parameters.
...
In the context of our analysis, the statistical evidence of a trend in transmission rate in Model~1 could be explained by any trending variable (such as hygiene improvements, or changes in population behavior), resulting in confounding from colinear covariates.
...
Here, we obtain forecasts by simulating future values using latent states that are consistent with the most recent observations by drawing latent states at time the last observation time $t_N$ from the filtering distribution $f_{\bm{X}_N|\bm{Y}_{1:N}}(\bm{x}_{N} | \bm{y}^*_{1:N} ; \hat\paramVec)$,
