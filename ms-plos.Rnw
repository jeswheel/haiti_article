...
Per-capita transition rates are given in Eqs.~\myeqref{eq:model3:SI}--\myeqref{eq:model3:Decay}.
\begin{eqnarray}
\label{eq:model3:SI}
...
The large difference in likelihoods that were obtain by these two approaches (see Table~\ref{tab:likes}) can primarily be attributed to increasing the computational effort in fitting the model; this result highlights the importance of carefully determining the necessary computational effort needed to maximize model likelihoods and acting accordingly.
