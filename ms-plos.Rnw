...
We build on a multi-group modeling exercise by \cite{lee20} in which four expert modeling teams developed models to the same dataset with the goal of comparing conclusions on the feasibility of eliminating cholera by a vaccination campaign.
...
Their conclusion is inconsistent with the prolonged period with no confirmed cholera cases between February, 2019 and September, 2022 \cite{trevisin22}, though cholera has recently reemerged in Haiti \cite{rubin22}.

The discrepancy between the model based conclusions of \cite{lee20} and the prolonged absence of cholera in Haiti has been debated \cite{francois20,rebaudetComment20,henrys20,leeReply20}.
...
The models make various choices on the dichotomies considered by \cite{lee20}; deterministic versus stochastic; compartment model versus agent-based; aggregated versus spatially explicit.
This is no accident, since \cite{lee20} purposefully designed their study to cover the range of current modeling practice.
...
\cite{lucas76} pointed out that it is naive to predict the effects of an intervention on a given system based entirely on historical associations.
...
The four mechanistic models of \cite{lee20} were deliberately developed with limited coordination.
...
Translations back to the original notation of \cite{lee20} are given in Table~S-1 of the supplement.
...
Per-capita transition rates are given in Eqs.~\ref{model1:SE}-\ref{model1:birth}:
\begin{eqnarray}
\label{model1:SE}
...
Here, $\childReduce = 0.4688$ represents the reduced efficacy of the vaccination for children under the age of five years, and the values $0.429$ and $0.519$ are the median effectiveness of one and two doses over their effective period respectively, according to Table S4 in the supplement material of \cite{lee20}.
...
To our knowledge, the only plug-and-play methods that can maximize the likelihood for POMP models of this complexity are iterated filtering algorithms \cite{ionides15}, which modify the well-known \emph{particle filter} \cite{arulampalam02}.
...
We implemented Model~1 using the \code{pomp} package \cite{king16}, relying heavily on the \code{pomp} source code provided by \cite{lee20}.
...
However, a legitimate preference for simplicity \cite{saltelli20,green15} can support a decision to consider nationally aggregated models.
...
To avoid the approximation error present in high-dimensional models, \cite{lee20} simplified the problem of estimating the parameters of Model~3 by creating an approximate version of the model where the units are independent given the observed data.
...
To make progress, while accepting proper limitations, one must bear in mind the much-quoted observation of \cite{box79} that ``all models are wrong but some are useful.''
...
That may explain why \cite{lee20} chose to obtain forecasts by simulating the calibrated models forward from initial conditions.
...
The primary forecasting goal of \cite{lee20} was to investigate the potential consequences of vaccination interventions on a system to inform policy.
...
We used the same data and models, and even much of the same code, as \cite{lee20}, and yet ended up with drastically different conclusions.
