...
The other main change in the revised manuscript is edits to the introduction and discussion, as recommended by both reviewers. These changes help make the goals of the mansucript clear, and connect the specific case study that was conducted to these goals. This resulted in improvements to the article without modifying the overall content of the original submission. 
...
Our primary changes in the revised manuscript include edits to the abstract, author summary, and introduction, with the goal of making the focus of the paper more clear.
...
Specifically, though the content and arguments made in the discussion remain the same, we point out ways that our case study demonstrates the usefulness of our general suggestions for model based inference of dynamic systems.
...
\article{We assess current methodological approaches to these issues via a case study of the 2010-2019 cholera epidemic in Haiti. We consider three dynamic models developed by expert teams to advise on vaccination policies. We evaluate previous methods used for fitting these models, and we demonstrate modified data analysis strategies leading to improved statistical fit.
Specifically, we present approaches for diagnosing model misspecification and the consequent development of improved models. Additionally, we demonstrate the utility of recent advances in likelihood maximization for high-dimensional nonlinear dynamic models, enabling likelihood-based inference for high-dimensional models for spatiotemporal incidence data.}
...
\article{In this article, we conduct a re-analysis of the 2010-2019 cholera outbreak in Haiti. We use this case study to investigate current procedures for fitting mechanistic models to time series data, while identifying limitations of these methodologies and proposing remedies.}
...
In the same table, however, they do not list $v_{rate}$ as a parameter that was ``fit", making it unclear where the value comes from.
...
A full discussion on parameters that are partially estimated via subjective measures is outside the scope of our article. To clarify where this fixed value comes from the revised manuscript reads:
...
\article{Models that are fit to the same datasets can be directly compared using AIC values, making it a useful tool to compare to benchmark models. Though Models~2 and 3 are both fit to department level incidence reports, their AIC values are not directly comparable due to the way Model~3 initializes latent states (\nameref{S_init}).}
...
The revised version of the paper contains significant edits to the introduction and discussion sections that we feel address this concern. In the discussion, the revised mansucript specifically mentions how the recalibrated models suggest a moderately high probability of cholera elimination from 2019-2022, which is consistent with what actually occured. We also discuss scientific insights that can made with the recalbrated models, such as the effect of Hurricane Matthew on cholera incidence, the importance of rainfall as a driver of cholera infection in Haiti, and the presence of overdispersion in the cholera dynamics.
...
The referee's suggestions have led to the addition of various references in the revision, which now includes over 100 citations.
...
This addition is intended to compliment the discussion about mechanistic (white-box) vs associative (black-box) models that was part of the ``Materials and methods" section.
...
In our study, we find that the deterministic model (model 2) has poor statistical fit (Table~2).
...
\article{Despite their limitations, deterministic models can offer valuable insights into dynamic systems \cite{may04}. In \cite{lee20}, the forecasts from the deterministic Model~2 were qualitatively more consistent with the observed disappearance of cholera than the stochastic models.}
...
% Figs S4, S5, S6, S7. \TODO{Check all figure numbers, especially to the supplement.}
...
MCAP methods have been used in various recent investigations using particle filters \cite{subramanian21,ali22,pons-salort18,simpson23}.
...
A full exposition of this topic is out of the scope of this paper, though some of these diagnostics are included in supplement material (\nameref{S_mod3cal}, \nameref{S_CI}).
To further help the reader in this regard, we have added additional information in the Model~1 calibration subsection, which includes citations to papers that discuss these diagnostics or have extensive supplements that demonstrate these diagnostic issues in action. 

\article{One change we made in the statistical analysis that led to larger model likelihoods was increasing the computational effort in the numerical maximization. While IF2 enables parameter estimation for a large class of models, the theoretic ability to maximize the likelihood depends on asymptotics in both the number of particles and the number of filtering iterations. Many Monte Carlo replications are then required to quantify and further reduce the error. The large increase in the log-likelihood for Model~1 (Table~2) can primarily be attributed to increasing the computational effort used to calibrate the model.
...
% Finally, we agree that we must be careful in drawing conclusions from any model, and that it is important acknowledge possible limitations. In our original manuscript, we explicitly noted that we should be careful drawing strong conclusions drawn from Model~3 due to some hints of model misspecification (\nameref{S_mod3cal}). This is clarified further in the revised manuscript: 

% \article{\editSuppText}

% Shorter term forecasts obtained by conditioning on previous data can be obtained by simulating from the filtering distribution.
...
We have added several paragraphs in the discussion to address this point. These paragraphs are provided below for reference.
...
\article{Despite their autonomy, the four independent teams obtained a consensus that an extensive nationwide vaccination campaign would be necessary to eliminate cholera from Haiti, estimating that a large number of cumulative cholera cases would be observed in the absense of additional vaccination efforts (Figure~3 and 4 of \cite{lee20}).
...
Due to the size and complexity of the table we have elected to put uncertainty estimates elsewhere (\nameref{S_CI}). This additional supplement material document outlines how confidence intervals are obtained, and includes tables and figures that show parameter uncertainty estimates. In the caption for Table~1, we include a reference to this supplement material.
...
Unsurprisingly, fitting initial values results in a quantitative improvement to model-fit, measured by AIC, but we also found that this had negligable qualitative effects on model-based conclusions. Importantly, the fitted model still falls short of the simple statistical benchmark used for comparison (\nameref{S_init}). 
We have included additional material in the supplement for Initialization Models that demonstrates this. 
On a nationally aggregated scale, the total number of initial infectious individuals in the fixed scenario was 36500, and 33480 when the initial states were estimated, which perhaps explains why calibrating initial parameters had little effect on qualitative conclusions. 
...
Because one of our goals was to demonstrate that the models of \cite{lee20} were alone not responsible for their poor forecasts, we elected to use the same initialization approach to the extent possible, as noted in the manuscript. 

The goal of inferring initial prevalence is to reduce the number of parameters.
...
In our supplement material (\nameref{S_mod3cal}), we discuss investigating conditional log-likelihoods of each observation in order to detect model misspecification. Here, we do not see clear evidence that fixing initial values in the same way that was done by \cite{lee20} results in poor fits of the first few available observations. 
Specifically, we found that enforcing model dynamics for initial states worked well for many of the departments, but was problematic for departments where this resulted in zero counts in Infected, Asymptomatic, and Bacterial compartments, leading to the estimation of initial counts of Infected individuals in two departments (Grand'Anse and Nippes) (units 3 and 4, respectively). See Table~1 for estimates of initial values for these two units.  
...
\article{The large increase in the log-likelihood for Model~1 (Table~2) can primarily be attributed to increasing the computational effort used to calibrate the model.
This result highlights the importance of carefully determining the necessary computational effort needed to maximize model likelihoods and acting accordingly.
In this case study, this was done by performing standard diagnostics for the IF2 and particle filter algorithms \cite{king16,li23,pons-salort18,laneri10}. Given the considerable computational costs of simulation-based algorithms, we find it useful to perform an initial assessment using hyperparameter values---such as the number of particles, filtering iterations, and replicates based on different parameter initializations---that enable relatively quick calculations. The insights obtained from this preliminary analysis help in accurately determining the amount of computation that is required to achieve reliable outcomes.}
