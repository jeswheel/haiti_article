...
Additional commentary to this point has been added in the abstract, introduction, and \TODO{the subsection on fitting Model 3}. 
...
% We have clarified this in the abstract and introduction by mentioning that we demonstrate the utility of recent advances in likelihood maximization for high-dimensional nonlinear dynamic models. 
% \TODO{Furthermore, we have added additional commentary that clearly indicates that although this case study is among the first to use the nov
...
We have also added citations to \cite{he10,stocks20} \TODO{and \cite{li23}} for examples demonstrating situations where overdispersed dynamic noise is present, and \cite{king15} for an investigation of the statistical limitations of fitting deterministic models to data.
...
\article{The statistical evidence of a trend in transmission rate in this model could be explained by any trending variable (such as hygiene improvements, or changes in population behavior), resulting in confounding from collinear covariates.
Because we only have access to incidence data, the decrease in observed cases over the course of the outbreak may alternatively be attributed to a trend in other model parameters, such as a decrease in the reporting rate over time.
This results in confounded mechanisms, as likelihood-based comparisons of a model that permits a change in reporting rate against a model that permits a change in transmission rate may not yield definitive results. In such a case, the robust statistical conclusion is that a model which allows for change fits better than one which does not, but the incidence data themselves may not provide enough information to pin down the mechanism.
In this case study, we did not explicitly test if the incidence data favor a trend in transmission over a trend in reporting rate. We argue that a decreasing transmission rate is a plausible way to explain the decrease in cases over time, as there is alternative evidence that supports this model \cite{rebaudet19CATI,rebaudet21,michel19}. Additionally there is little scientific evidence to support a decrease in reporting rate (for example, see Figure~1 of \cite{rebaudet21}).}
...
\jwc{It doesn't immediately come into play in our analysis a lot, so it might be good to point to a reference suce as saltelli19, who has a sketch of what they believe on dynamic modeling: adding model complexity is good because it becomes more realistic, but too much complexity results in more ``propogation error"}
